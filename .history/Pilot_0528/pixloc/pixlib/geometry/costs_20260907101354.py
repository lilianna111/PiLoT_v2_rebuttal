...
# 计时开关：设环境变量 PILOT_SYNC_TIMING=1 时，每次取时间戳前先 cuda.synchronize()，
# 使分项计时落到真实 GPU 计算上（默认关闭，生产零开销，不改变任何计算结果）。
_SYNC_TIMING = os.environ.get("PILOT_SYNC_TIMING", "0") == "1"


def _sync_now():
    if _SYNC_TIMING and torch.cuda.is_available():
        torch.cuda.synchronize()
    return time.perf_counter()


# 懒加载融合投影 CUDA 扩展(DirectAbsoluteCostCuda/)。首次调用时导入, 失败则回退纯 Python。
_P2DR_EXT = None
def _load_p2dr_ext():
    global _P2DR_EXT
    if _P2DR_EXT is None:
        import importlib, sys
        from pathlib import Path
        ext_dir = Path(__file__).resolve().parents[3] / "DirectAbsoluteCostCuda"
        if str(ext_dir) not in sys.path:
            sys.path.insert(0, str(ext_dir))
        _P2DR_EXT = importlib.import_module("direct_abs_cost_cuda")
    return _P2DR_EXT


def _ecef_to_wgs84_torch(X, Y, Z):
    a = 6378137.0; f = 1.0/298.257223563; b = a*(1-f); e2 = f*(2-f); ep2 = e2/(1-e2)
    p = torch.sqrt(X*X + Y*Y); th = torch.atan2(Z*a, p*b); lon = torch.atan2(Y, X)
    lat = torch.atan2(Z + ep2*b*torch.sin(th)**3, p - e2*a*torch.cos(th)**3)
    N = a/torch.sqrt(1 - e2*torch.sin(lat)**2); h = p/torch.cos(lat) - N
    return torch.rad2deg(lon), torch.rad2deg(lat), h


def _wgs84_to_tm4547_torch(lon_deg, lat_deg):
    a = 6378137.0; f = 1.0/298.257222101; k0 = 1.0
    lon0 = float(np.radians(114.0)); x0 = 500000.0
    e2 = f*(2-f); ep2 = e2/(1-e2)
    lat = torch.deg2rad(lat_deg); lon = torch.deg2rad(lon_deg)
    s2 = torch.sin(lat); c2 = torch.cos(lat); t2 = torch.tan(lat)
    N = a/torch.sqrt(1 - e2*s2*s2); T = t2*t2; C = ep2*c2*c2; A = (lon - lon0)*c2
    ...
    x = k0*N*(A + (1-T+C)*A**3/6 + (5-18*T+T*T+72*C-58*ep2)*A**5/120) + x0
    y = k0*(M + N*t2*(A*A/2 + (5-T+9*C+4*C*C)*A**4/24
...
class DirectAbsoluteCost2:
    def __init__(self):
        ...
        # 融合投影 CUDA 核开关(消除697次kernel启动, 精度已验证匹配Python<1e-6)
        self.use_cuda_p2dr = False
        # 启用深度约束（DSM prior）。
        self.enable_depth_prior = True
        # True False
    ...
    def _center_lonlatalt_torch(self, pose_data_q, depth, K, origin, dd, render_T_ecef):
        if K is None:
            K = [1920, 1080, 1350.0, 1350.0, 960.0, 540.0]
        _, _, fx, fy, cx, cy = K
        depth = float(depth)
        ...
        if origin is not None and dd is not None:
            origin_t = torch.as_tensor(origin, device=dev, dtype=torch.float64)
            dd_t = torch.as_tensor(dd, device=dev, dtype=torch.float64)
            C_ecef = C_local + origin_t + dd_t
        else:
            C_ecef = C_local + torch.as_tensor(render_T_ecef, device=dev, dtype=torch.float64)
        # 中心像素反投影到光轴: K_inv @ [cx,cy,1]==[0,0,1](恒等), 故 cam_vec=[0,0,depth],
        # ray_world = R_c2w @ [0,0,depth] = depth * R_c2w[:,:,2]。免去 torch.inverse 与张量构造。
        ray_world = R_c2w[:, :, 2] * depth
        point_ecef = C_ecef + ray_world
        lon, lat, h = _ecef_to_wgs84_torch(point_ecef[:, 0], point_ecef[:, 1], point_ecef[:, 2])
        return torch.stack([lon, lat, h], dim=-1)

    def _query_dsm_heights_torch(self, lon_t, lat_t, dsm_path, proj_crs="EPSG:4547"):
        cache = self._dsm_cache.get(dsm_path)
        if cache is None or cache.get("proj_crs") != proj_crs or cache.get("gpu") is None:
            self._query_dsm_heights([[114.0, 28.0]], dsm_path, proj_crs=proj_crs, use_gpu=True)
            cache = self._dsm_cache.get(dsm_path)
        mt = cache["gpu"]["map_tensor"]; gt = cache["geotransform"]
        rows, cols = cache["rows"], cache["cols"]
        x_proj, y_proj = _wgs84_to_tm4547_torch(lon_t, lat_t)
        col_f = (x_proj - gt[0]) / gt[1]; row_f = (y_proj - gt[3]) / gt[5]
        valid = (col_f >= 0) & (col_f < cols) & (row_f >= 0) & (row_f < rows)
        grid_u = (col_f / (cols - 1)) * 2.0 - 1.0; grid_v = (row_f / (rows - 1)) * 2.0 - 1.0
        grid = torch.stack([grid_u, grid_v], dim=-1).view(1, -1, 1, 2).to(dtype=mt.dtype)
        sample = F.grid_sample(mt, grid, align_corners=True, padding_mode="border")
        return sample.view(-1).to(dtype=torch.float64).masked_fill(~valid, float("nan"))
    ...
    def interpolate_feature_map(
        ...
        # 原生融合 CUDA 核，等价于自写 bilinear_grid_sample(align_corners=True, zero-pad)，已验证数值差 <5e-7
        fp = F.grid_sample(feature, pts[:, None], mode='bilinear', align_corners=True, padding_mode='zeros')
        ...
        if return_gradients:
            ...
            tensor_d = F.grid_sample(
                    feature, pts_d[:, None], mode='bilinear', align_corners=True, padding_mode='zeros')
            ...
            gradients = None  # 不需要梯度时不分配巨型全零张量(cost_only 下省 100+MB, 结果不变)
    ...
    def residual_jacobian_batch_quat(
        ...
        w_depth=None, w_angle=None, cost_only=False):
        """
        ...
        # -------- 融合投影 CUDA 核: 一次算出投影的 grad/Hess/cost/valid, 消除697次kernel启动 --------
        _cuda_ok = False
        neg_grad_cuda = Hess_cuda = cost_raw_cuda = valid_cuda = None
        res = None
        J = None
        p2d_q = None
        if getattr(self, "use_cuda_p2dr", False):
            try:
                _ext = _load_p2dr_ext()
                neg_grad_cuda, Hess_cuda, cost_raw_cuda, valid_cuda = _ext.residual_jacobian_p2dr_cuda(
                    pose_data_q.contiguous().float(),
                    f_r.contiguous().float(),
                    f_q.contiguous().float(),
                    cam_data_q.contiguous().float(),
                    p3D.reshape(1, -1, 3).contiguous().float(),
                    c_ref.contiguous().float(),
                    c_query.contiguous().float(),
                    p2d_r.reshape(1, -1, 2).contiguous().float(),
                    visible_r.reshape(1, -1).float().contiguous(),
                )
                _cuda_ok = True
                p2d_q = torch.zeros(1, num_init_pose, cost_raw_cuda.shape[-1], 2,
                                    device=neg_grad_cuda.device, dtype=neg_grad_cuda.dtype)
            except Exception:
                _cuda_ok = False
        ...
        if not _cuda_ok:
            ...
            fp_q, valid_q, J_f = self.interpolate_feature_map(
                f_q, p2d_q.reshape(1, -1, 2), return_gradients=(not cost_only)
            )
        ...
        timing['proj_ms'] = float((_sync_now() - proj_t0) * 1e3)
        # 深度/角度先验仅用 j_ref 的 device/dtype。cuda 路径用 neg_grad_cuda, 否则用 J 或 res。
        j_ref = neg_grad_cuda if _cuda_ok else (J if J is not None else res)
        ...
        if (
            ...
            and gt_depth is not None
            and dsm_path is not None
            and not (cost_only and float(self.depth_selection_weight) == 0.0)
        ):
            depth_t0 = _sync_now()
            if torch.is_tensor(gt_depth):
            ...
            # === 全 GPU 深度: base+FD 批处理, Bowring+torchTM+grid_sample, 无 pyproj ===
            eps_fd_rot = np.deg2rad(float(self.depth_fd_eps_rot_deg))
            eps_fd_trans = float(self.depth_fd_eps_trans)
            J_depth_t = torch.zeros(num_init_pose, 6, device=pose.device, dtype=torch.float64)
            fd_cols = [] if cost_only else ([3, 4, 5] if self.depth_translation_only else [0, 1, 2, 3, 4, 5])
            pose_sets = [pose_data_q if pose_data_q.ndim == 3 else pose_data_q.unsqueeze(0)]
            eps_per_col = []
            for k in fd_cols:
                dR = torch.eye(3, device=pose.device, dtype=torch.float64).unsqueeze(0).repeat(num_init_pose, 1, 1)
                ...
                if k < 3:
                    ...
                    dvec[:, k] = eps_fd_rot; dR = so3exp_map(dvec); eps_per_col.append(eps_fd_rot)
                else:
                    dt[:, k - 3] = eps_fd_trans; eps_per_col.append(eps_fd_trans)
                R_pert = torch.bmm(R_w2c, dR)
                t_pert = torch.bmm(R_w2c, dt.unsqueeze(-1)).squeeze(-1) + t_w2c
                pose_sets.append(torch.cat([R_pert.reshape(num_init_pose, -1), t_pert], dim=-1).unsqueeze(0))
            poses_all = torch.cat(pose_sets, dim=1)
            depth_center_t0 = _sync_now()
            lla_all = self._center_lonlatalt_torch(poses_all, depth_real, cam_K, origin, dd, render_T_ecef)
            timing['depth_center_ms'] += float((_sync_now() - depth_center_t0) * 1e3)
            depth_dsm_t0 = _sync_now()
            dsm_all = self._query_dsm_heights_torch(lla_all[:, 0], lla_all[:, 1], dsm_path)
            timing['depth_dsm_ms'] += float((_sync_now() - depth_dsm_t0) * 1e3)
            res_all = lla_all[:, 2] - dsm_all
            base_residuals = torch.nan_to_num(res_all[:num_init_pose], nan=0.0)
            valid_mask = ~torch.isnan(res_all[:num_init_pose])
            base_raw = base_residuals.clone()
            depth_fd_t0 = _sync_now()
            for idx, k in enumerate(fd_cols):
                r_pert = res_all[(idx + 1) * num_init_pose:(idx + 2) * num_init_pose]
                vb = valid_mask & (~torch.isnan(r_pert))
                r_pert = torch.nan_to_num(r_pert, nan=0.0)
                J_depth_t[:, k] = torch.where(vb, (r_pert - base_raw) / eps_per_col[idx], torch.zeros_like(base_raw))
            timing['depth_fd_ms'] += float((_sync_now() - depth_fd_t0) * 1e3)
            J_depth_t = torch.nan_to_num(J_depth_t, nan=0.0)
            if self.depth_translation_only:
                J_depth_t[:, :3] = 0.0
            base_residuals = base_residuals / DEPTH_NORM
            J_depth_t = J_depth_t / DEPTH_NORM

            depth_res = base_residuals.to(device=j_ref.device, dtype=j_ref.dtype).view(1, num_init_pose, 1)
            J_depth_tensor = J_depth_t.to(device=j_ref.device, dtype=j_ref.dtype).view(1, num_init_pose, 1, 6)
            valid_depth = valid_mask.to(device=j_ref.device, dtype=j_ref.dtype).view(1, num_init_pose, 1)

            if self.depth_debug_verbose and self.iter <= 3:
                r_m = (base_residuals * DEPTH_NORM).detach().cpu().numpy()
                print(
            ...
            timing['depth_ms'] = float((_sync_now() - depth_t0) * 1e3)

        # 角度约束：由 enable_angle_prior 控制，打开时会与深度约束一起并入优化。
        angle_res = None
        J_angle_tensor = None
        angle_weight = 1.0

        # === Step 6: Angle prior ===
        # 当 enable_angle_prior=False 时，angle_res / J_angle_tensor 保持 None。
        #
        # 角度 prior 的目标：
        ...
        if (
            ...
            angle_setup_t0 = _sync_now()
            # ECEF -> (lon,lat) 用 GPU Bowring(替代 pyproj CPU 往返), 只为构建局部 ENU 基底。
            # 与 pyproj 差 <1e-14 度, ENU 基底与角度残差精度不变。
            C_ecef2 = C_ecef if C_ecef.ndim == 2 else C_ecef.view(1, 3)
            lon_deg_t, lat_deg_t, _ = _ecef_to_wgs84_torch(
                C_ecef2[:, 0], C_ecef2[:, 1], C_ecef2[:, 2]
            )
            lat_rad_t = torch.deg2rad(lat_deg_t)
            lon_rad_t = torch.deg2rad(lon_deg_t)
            ...
            rot_enu_in_ecef = torch.stack([east, north, up], dim=-1)
            ...
            timing['angle_setup_ms'] += float((_sync_now() - angle_setup_t0) * 1e3)
            ...
            angle_fd_t0 = _sync_now()
            # cost_only 只需 angle_res(来自 base_roll/pitch), 不需雅可比, 跳过整个 FD。
            if not cost_only:
                # 3 个 FD 列批处理: 拼成 [3*N] 一次 so3exp + bmm, 免 3 次小核循环。结果与逐列一致。
                dvec_all = torch.zeros(3, num_init_pose, 3, device=pose.device, dtype=torch.float64)
                for k in range(3):
                    dvec_all[k, :, k] = eps_fd
                dvec_all = dvec_all.reshape(3 * num_init_pose, 3)
                dR_all = so3exp_map(dvec_all)                              # [3N,3,3]
                R_w2c_rep = R_w2c.unsqueeze(0).expand(3, -1, -1, -1).reshape(3 * num_init_pose, 3, 3)
                R_w2c_pert = torch.bmm(R_w2c_rep, dR_all)
                R_c2w_pert = R_w2c_pert.transpose(-1, -2).clone()
                R_c2w_pert[:, :, 1] *= -1
                R_c2w_pert[:, :, 2] *= -1
                rot_enu_T_rep = rot_enu_in_ecef_T.unsqueeze(0).expand(3, -1, -1, -1).reshape(3 * num_init_pose, 3, 3)
                R_pose_pert = torch.bmm(rot_enu_T_rep, R_c2w_pert)
                roll_pert_all, pitch_pert_all = _extract_roll_pitch_from_R_pose(R_pose_pert)
                roll_pert_all = roll_pert_all.reshape(3, num_init_pose)
                pitch_pert_all = pitch_pert_all.reshape(3, num_init_pose)
                for k in range(3):
                    droll = torch.remainder(roll_pert_all[k] - base_roll + np.pi, 2.0 * np.pi) - np.pi
                    dpitch = torch.remainder(pitch_pert_all[k] - base_pitch + np.pi, 2.0 * np.pi) - np.pi
                    J_roll_cols.append(droll / eps_fd)
                    J_pitch_cols.append(dpitch / eps_fd)
            timing['angle_fd_ms'] += float((_sync_now() - angle_fd_t0) * 1e3)

            J_roll = torch.stack(J_roll_cols, dim=-1) if not cost_only else None   # [num_init_pose, 3]
            J_pitch = torch.stack(J_pitch_cols, dim=-1) if not cost_only else None # [num_init_pose, 3]
            ...
            # 转换为与 J 相同的 dtype
            angle_res = angle_r.to(dtype=j_ref.dtype)  # [num_init_pose, 2]
            angle_res = angle_res.view(1, num_init_pose, 2)
            if not cost_only:
                J_angle_rot = torch.stack([J_roll, J_pitch], dim=1)  # [num_init_pose, 2, 3]
                J_angle_rot = J_angle_rot.to(dtype=j_ref.dtype)  # [num_init_pose, 2, 3]
                # 扩展 J_angle 到完整的 6 自由度（后 3 个平移分量为 0）：
                # 角度 prior 只约束旋转，因此 d(roll,pitch)/d(t)=0, d(roll,pitch)/d(ω)=J_angle_rot
                J_angle_full = torch.zeros(num_init_pose, 2, 6, device=j_ref.device, dtype=j_ref.dtype)
            ...
            timing['angle_ms'] = float((_sync_now() - angle_t0) * 1e3)

        # === Step 5: loss 和加权 ===
        system_t0 = _sync_now()
        cost_proj = cost_raw_cuda if _cuda_ok else (res**2).sum(-1)
        # cost, w_loss_proj, _ = self.loss_fn1(cost)

        valid_query_for_selection = valid_cuda if _cuda_ok else (valid_q & visible_q).float()
        ...
        if (depth_res is not None) or (angle_res is not None):
            # Deep prior must not share projection truncate directly.
            # Otherwise depth robust derivative becomes tiny and contribution collapses.
            cost_proj_robust, w_loss_proj, _ = self.loss_fn1(cost_proj_for_selection)
            cost_proj_robust = cost_proj_robust * valid_norm

            cost_terms = [cost_proj_robust]

            if depth_res is not None:
                # 深度约束 cost: (z_center - dsm_height)^2（已按 DEPTH_NORM 归一化）
                ...
                sel_gate = torch.where(
                    depth_abs_m_sel >= self.depth_gate_start_m, sel_linear, sel_gate
                )
            ...
            if angle_res is not None:
                ...
                cost_angle_robust, w_loss_angle, _ = self.loss_fn1(cost_angle)
                cost_terms.append(
                    float(self.angle_selection_weight) * valid_count * cost_angle_robust
                )

            cost = torch.cat(cost_terms, dim=-1)
        else:
            cost, w_loss_proj, _ = self.loss_fn1(cost_proj_for_selection)
            cost = cost * valid_norm

        if cost_only:
            # post 重评估只需要 cost；跳过权重插值、梯度与 Hessian 构建。
            # 其余返回值置位占位（调用方只取第 6 个返回值 cost）。
            timing['system_ms'] = float((_sync_now() - system_t0) * 1e3)
            timing['total_ms'] = float((_sync_now() - eval_t0) * 1e3)
            self.last_eval_timing = timing
            valid_query = valid_cuda if _cuda_ok else (valid_q & visible_q).float()
            return None, None, w_loss_proj, valid_query, p2d_q, cost

        valid_query = valid_cuda if _cuda_ok else (valid_q & visible_q).float()
        if _cuda_ok:
            grad_proj = -neg_grad_cuda
            Hess_proj = Hess_cuda
        else:
        ...
        if depth_res is not None and w_loss_depth is not None:
            ...
            grad = grad + grad_depth_scaled
            Hess = Hess + Hess_depth_scaled
        else:
        ...
        if angle_res is not None and w_loss_angle is not None:
            ...
            raw_scale = (
                self.angle_scale_fixed
                * (gn_proj / (gn_angle + eps_bal))
                * self.angle_target_ratio
            )
            angle_scale_per_pose = raw_scale.clamp(
                min=self.angle_scale_min, max=self.angle_scale_max
            )

            angle_norm_deg = torch.rad2deg(angle_res.norm(dim=-1, keepdim=True))
            ...
            if self.angle_debug_print:
                ...
                try:
                    grad_tmp = grad + grad_angle * angle_scale_per_pose
                    Hess_tmp = Hess + Hess_angle * angle_scale_per_pose.unsqueeze(-1)
                    eye6 = torch.eye(
                        6, device=Hess.device, dtype=Hess.dtype
                    ).view(1, 1, 6, 6)
                    H_damped = Hess_tmp + 1e-6 * eye6
                    delta_dbg = torch.linalg.solve(
                        H_damped, (-grad_tmp).unsqueeze(-1)
                    ).squeeze(-1)
                    J_angle_rot_dbg = J_angle_tensor[..., :, :3]
                    pred_dr = torch.einsum(
                        "bnij,bnj->bni", J_angle_rot_dbg, delta_dbg[..., :3]
                    )
                    dir_dot = (angle_res * pred_dr).sum(dim=-1)
