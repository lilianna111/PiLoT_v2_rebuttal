...
# CUDA 融合投影核(DirectAbsoluteCostCuda/*.so)依赖 PyTorch 的 libc10.so 等库,
# 它们在 torch/lib 而非 $CONDA_PREFIX/lib。不加此路径扩展加载会失败→回退纯Python(248ms)。
export LD_LIBRARY_PATH=$($CONDA_PREFIX/bin/python -c "import torch,os;print(os.path.join(os.path.dirname(torch.__file__),'lib'))"):$LD_LIBRARY_PATH
...
sample_nums=(100 300 700 900)
output_base="/media/amax/PS2000/rebuttal/0906"
...
for target_name in "${target_names[@]}"; do
  ...
  if [[ $index -ge 0 ]]; then
    ...
    for sample_num in "${sample_nums[@]}"; do
      output_folder="${output_base}/seed${sample_num}"

      echo "--- fpvloc sample_num=${sample_num}"
      echo "output_folder : $output_folder"
      python /home/amax/Documents/code/pilot_v2/pilot_v2_0702_60ms/Pilot_0528/main.py \
        --config "/home/amax/Documents/code/pilot_v2/pilot_v2_0702_60ms/Pilot_0528/configs/feicuiwan_m4t.yaml" \
        --init_euler "$euler" \
        --init_trans "$trans" \
        --name "$target_name" \
