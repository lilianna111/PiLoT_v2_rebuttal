...
class DualProcessTask:        
    def __init__(self, config, init_euler = None, init_trans = None, name = None):
        ...
        output_folder = "/media/amax/AE0E2AFD0E2ABE69/datasets/outputs/FPVLoc_angle_1"
                ...
                while True:
                    try:
                        ...
                        if self.stop_evt.is_set():
                            break
                        continue
            print(f"Localization Thread calculated frames: {idx} | Pitch, Roll, Yaw: {np.asarray(pred_euler).tolist()} | Longitude, Latitude, Altitude: {np.asarray(pred_trans).tolist()}")
            # 4) 暂存结果
