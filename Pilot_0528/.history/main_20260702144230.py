...
class DualProcessTask:        
    def __init__(self, config, init_euler = None, init_trans = None, name = None):
        ...
        # self.estimated_pose = os.path.join(folder_path, 'estimation', 'FPVLoc@'+output_name +'.txt') + dataset_name +'.txt'
        # output_folder = "/media/amax/AE0E2AFD0E2ABE69/datasets/outputs/FPVLoc_depth"
        output_folder = "/media/amax/PS2000/ral/crop_0702"
    ...
    def rendering_worker(self):
        ...
        ref_npy_path = "/media/amax/AE0E2AFD0E2ABE69/datasets/DSM/0.3/0.3/wgs84npy.npy"
    ...
    def localization_worker(self):
        ...
        if crop_times_ms and localization_times_ms:
            print(
                "[Sequence Timing] frames={} | crop_ms={:.2f} | loc_ms={:.2f}".format(
                    len(results),
                    float(np.sum(crop_times_ms)),
                    float(np.sum(localization_times_ms)),
                )
            )
