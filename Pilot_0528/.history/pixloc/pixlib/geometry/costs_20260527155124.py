...
class DirectAbsoluteCost2:
    def __init__(self):
        ...
        self._last_lm_timing = None
        self.lm_timing_print = True
        ...
        self.depth_selection_weight = 0.5
    ...
    def _sync_time(self, device=None) -> float:
        if self.lm_timing_print and device is not None and torch.is_tensor(device):
            device = device.device
        if self.lm_timing_print and device is not None and device.type == "cuda":
    ...
    def build_reference_cache(
        ...
        fp_r_base = fp_r[0] * mask
        p3d_base = p3D[0] * mask
        weight_r_base, _, _ = self.interpolate_feature_map(c_ref, p2d_r)
        weight_r_base = weight_r_base * mask.T.view(1, -1, 1)

        fp_r_full = torch.nn.functional.normalize(
            fp_r_base.unsqueeze(0).unsqueeze(0).expand(1, num_init_pose, -1, -1),
            dim=-1,
        )
        p3d_full = p3d_base.unsqueeze(0).unsqueeze(0).expand(1, num_init_pose, -1, -1)
        weight_r_full = weight_r_base.unsqueeze(1).expand(1, num_init_pose, -1, -1)
        return {
            "mask": mask,
            "fp_r": fp_r_full,
            "p3D": p3d_full,
            "weight_r": weight_r_full,
        }
    ...
    def residual_jacobian_batch_quat(
        ...
        w_depth=None, w_angle=None, ref_cache: Optional[dict] = None):
        """
        ...
        """
        timing_device = pose_data_q.device if torch.is_tensor(pose_data_q) else None
        t_total_start = self._sync_time(timing_device)
        t_marks = {}

        def _mark(name: str):
            t_marks[name] = self._sync_time(timing_device)
        ...
        if ref_cache is None:
            ref_cache = self.build_reference_cache(
                f_r, c_ref, p2d_r, visible_r, p3D, num_init_pose
            )
        mask = ref_cache["mask"]
        fp_r = ref_cache["fp_r"]
        p3D = ref_cache["p3D"]
        ...
        if (
            ...
            for k in range(6):
                ...
                pose_pert_list.append(pose_pert)
                eps_used_list.append(eps_used)

            pose_pert_all = torch.cat(pose_pert_list, dim=1)
            r_pert_all = _depth_residual_for_pose(pose_pert_all).reshape(6, num_init_pose)
            valid_pert_all = ~np.isnan(r_pert_all)
            eps_used_arr = np.asarray(eps_used_list, dtype=np.float64).reshape(6, 1)
            valid_both_all = valid_pert_all & valid_mask[None, :]
            J_depth_batch = np.zeros((6, num_init_pose), dtype=np.float64)
            valid_diff = valid_both_all
            if np.any(valid_diff):
                J_depth_batch[valid_diff] = (
                    (r_pert_all[valid_diff] - np.broadcast_to(base_residuals_raw, (6, num_init_pose))[valid_diff])
                    / np.broadcast_to(eps_used_arr, (6, num_init_pose))[valid_diff]
                )
            J_depth = J_depth_batch.T
            ...
            depth_device = pose_data_q.device
            depth_res = torch.from_numpy(base_residuals).to(device=depth_device, dtype=tensor_dtype).view(
                1, num_init_pose, 1
            )
            J_depth_tensor = torch.from_numpy(J_depth).to(device=depth_device, dtype=tensor_dtype).view(
                1, num_init_pose, 1, 6
            )
            valid_depth = torch.from_numpy(valid_mask.astype(np.float32)).to(
                device=depth_device, dtype=tensor_dtype
        ...
        if (
            ...
            wgs84_pose = ECEF_to_WGS84(C_ecef.detach().cpu().numpy())
            lon_deg = wgs84_pose[:, 0]
            lat_deg = wgs84_pose[:, 1]
            rot_enu_in_ecef_np = np.stack(
                [get_rotation_enu_in_ecef(lon, lat) for lon, lat in zip(lon_deg, lat_deg)],
                axis=0,
            )
            rot_enu_in_ecef = torch.as_tensor(
                rot_enu_in_ecef_np,
                device=pose.device,
                dtype=torch.float64,
            )
            ...
            # 转换为与 J 相同的 dtype
            angle_res = angle_r.to(dtype=tensor_dtype)  # [num_init_pose, 2]
            J_angle_rot = J_angle_rot.to(dtype=tensor_dtype)  # [num_init_pose, 2, 3]
            
            # 扩展 J_angle 到完整的 6 自由度（后 3 个平移分量为 0）：
            # 角度 prior 只约束旋转，因此：
            #   d(roll,pitch)/d(t) = 0
            #   d(roll,pitch)/d(ω) = J_angle_rot
            J_angle_full = torch.zeros(num_init_pose, 2, 6, device=pose.device, dtype=tensor_dtype)
        ...
        self._last_lm_timing = {
            "transform_project_s": t_marks["transform_project"] - t_marks["start"],
            "query_interp_s": t_marks["query_interp"] - t_marks["transform_project"],
            "jacobian_s": t_marks["reproj"] - t_marks["query_interp"],
            "reproj_s": t_marks["reproj"] - t_marks["start"],
            "depth_s": t_marks["depth"] - t_marks["reproj"],
            "angle_s": t_marks["angle"] - t_marks["depth"],
            "loss_s": t_marks["loss"] - t_marks["angle"],
            "proj_gh_s": t_marks["proj_gh"] - t_marks["loss"],
            "depth_gh_s": t_marks["depth_gh"] - t_marks["proj_gh"],
            "angle_gh_s": t_marks["angle_gh"] - t_marks["depth_gh"],
            "total_s": self._sync_time(timing_device) - t_total_start,
            "num_pose": int(num_init_pose),
            "num_points": int(p3D.shape[-2]),
            "depth_on": depth_res is not None,
            "angle_on": angle_res is not None,
        }
