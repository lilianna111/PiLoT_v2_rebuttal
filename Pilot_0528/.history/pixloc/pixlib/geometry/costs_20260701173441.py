...
class DirectAbsoluteCost2:
    ...
    def residual_jacobian_batch_quat(
        ...
        # cost_only 时 J 为 None；深度/角度先验仅用其 device/dtype，回退到同源的 res（与 J 同源自 f_q，device/dtype 一致）。
        j_ref = J if J is not None else res
        ...
        if (
            ...
            for k in range(6):
                # cost_only 时不需要雅可比，跳过全部有限差分前向（含 DSM 查询）。
                # depth_translation_only 时旋转 3 列会在循环后被置零（见下方 J_depth[:, :3] = 0.0），
                # 故跳过它们的有限差分前向，结果逐位不变。
                if cost_only or (k < 3 and self.depth_translation_only):
                    continue
                ...
                fd_eval_t0 = _sync_now()
                r_pert = _depth_residual_for_pose(pose_pert)
                timing['depth_fd_ms'] += float((_sync_now() - fd_eval_t0) * 1e3)
            ...
            depth_res = torch.from_numpy(base_residuals).to(device=j_ref.device, dtype=j_ref.dtype).view(
                1, num_init_pose, 1
            )
            J_depth_tensor = torch.from_numpy(J_depth).to(device=j_ref.device, dtype=j_ref.dtype).view(
                1, num_init_pose, 1, 6
            )
            valid_depth = torch.from_numpy(valid_mask.astype(np.float32)).to(
                device=j_ref.device, dtype=j_ref.dtype
