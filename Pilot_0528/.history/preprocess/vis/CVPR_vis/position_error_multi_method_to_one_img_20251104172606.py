...
def draw_points_on_image_multimethod(image, multi_method_xy, multi_method_error, method_styles, point_radius=12):
    ...
    for method_name, xy_dict in multi_method_xy.items():
        ...
        # 遍历该方法的所有目标点
        for target_name, coord in xy_dict.items():
            if not coord or len(coord) == 0: continue
...
def get_es_xyz(save_xyz_path):
    ...
    with open(save_xyz_path, 'r') as file:
        ...
        for parts in reader:
            if len(parts) >= 5:
                img_name, car_name = parts[0], parts[1]
                # 这里假设目标点是固定的，只取一个car的所有img_name
                # 为了简化，我们只处理一个car_name的情况
                if car_name not in xyz_dict:
                    xyz_dict[car_name] = {}
                xyz_dict[car_name][img_name] = list(map(float, parts[2:5]))
    return xyz_dict
...
def main():
    ...
    gt_path_root = "/mnt/sda/MapScape/query/poses/"
    ...
    for seq in txt_files:
        ...
        for method_key, subfolder in methods.items():
            ...
            # 确定结果文件路径
            result_file = os.path.join(gt_path_root, f"{seq}.txt") if method_key == 'GT' else os.path.join(base_path, method_key, f"{seq}.txt")
            
            # 读取XYZ坐标
            xyz_data_all_cars = get_es_xyz(result_file)
            
            # 筛选出我们关心的目标
            if TARGET_CAR_NAME in xyz_data_all_cars:
                target_xyz_wgs = xyz_data_all_cars[TARGET_CAR_NAME].get(VIEW_FRAME_IMG_NAME)
                
                if target_xyz_wgs:
                    # 存储WGS84坐标
                    if method_key not in multi_method_xyz: multi_method_xyz[method_key] = {}
                    multi_method_xyz[method_key][TARGET_CAR_NAME] = target_xyz_wgs
                    ...
                    if xy_proj is not None and len(xy_proj) > 0:
                        if method_key not in multi_method_xy_proj: multi_method_xy_proj[method_key] = {}
                        multi_method_xy_proj[method_key][TARGET_CAR_NAME] = xy_proj[0] # 取第一个点
                else:
                    print(f"    [WARN] No data for frame '{VIEW_FRAME_IMG_NAME}' for target '{TARGET_CAR_NAME}' in {method_key}")
            else:
                print(f"    [WARN] No data for target '{TARGET_CAR_NAME}' in {method_key}")
