...
def main():
    ...
    MARKER_OUTER_SHAPE = 's'       # 外框形状：'s' (方形), 'o' (圆形), 'D' (菱形)
    MARKER_OUTER_SIZE  = 10        # 外框大小
    MARKER_INNER_SHAPE = 'o'       # 内点形状
    MARKER_INNER_SIZE  = 3         # 内点大小
    MARKER_INNER_COLOR = (0, 0, 0) # 内点颜色，统一用黑色以突出对比
    # --- 2. 定义每个方法独有的颜色 ---
    # 字典变得极其简洁，只负责颜色区分
    method_color_map = {
    'GT':              (0, 255, 0),    # 绿色
    'Render2loc@raft': (255, 0, 0),    # 红色
    'FPVLoc':          (0, 255, 255),  # 青色
    'Pixloc':          (255, 0, 255),  # 品红
    'Render2loc':      (0, 0, 255),    # 蓝色
    'Render2ORB':      (255, 165, 0),  # <-- 新增：为 Render2ORB 添加橙色 (Orange)
}
