...
def main():
    ...
    method_styles = {
        'GT':              {'shape': 'circle',   'color': (255, 0, 0)},   # 绿色 圆形
        'Render2loc@raft': {'shape': 'square',   'color': "#F7B84A"},   # 蓝色 方形
        'FPVLoc':          {'shape': 'star', 'color': '#007F49'}, # 黄色 三角形
        'Pixloc':          {'shape': 'circle', 'color': "#86AED5"}, # 品红 圆形 (与GT形状相同，颜色不同)
        'Render2loc':      {'shape': 'triangle',  'color': "#EF6C5D"},      # <-- 新增：蓝色 菱形 (Blue)
        'Render2ORB':      {'shape': 'triangle',  'color': "#C79ACD"},      # <-- 新增：蓝色 菱形 (Blue)
    }
