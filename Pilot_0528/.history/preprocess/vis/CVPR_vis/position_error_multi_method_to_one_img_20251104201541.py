...
# (新修改) 绘制图例的辅助函数
def putText_pil(image, text, position, font_path, font_size, color):
...
# =====================================================================
#  ↓↓↓ 使用这个修正后的 draw_legend 函数 ↓↓↓
# =====================================================================
def draw_legend(image, legend_items, top_left_corner, font_path, font_size=20, bg_color=(0, 0, 0, 0.5)):
    """
    在图像上绘制一个对齐的、带背景的图例。
    ...
    :param font_path: Times New Roman 等 .ttf 字体文件的路径。
    :param font_size: 字体大小。
    :param bg_color: 背景颜色和透明度 (B, G, R, Alpha)，Alpha 介于 0.0 (完全透明) 到 1.0 (不透明)。
    """
    # --- 1. 定义布局参数 ---
    x, y = top_left_corner
    padding = 15          # 内部边距
    line_height = 40      # 每行的高度
    icon_radius = 11      # 图标的半径 (用于计算空间)
    ...
    for name, _, _ in legend_items:
        # Pillow >= 10.0.0
        if hasattr(temp_font, 'getbbox'):
            text_width = temp_font.getbbox(name)[2]
        # Pillow < 10.0.0
        else:
            text_width = temp_font.getsize(name)[0]
        if text_width > max_text_width:
            max_text_width = text_width
    legend_width = text_start_x - x + max_text_width + padding
    legend_height = len(legend_items) * line_height + padding * 2 - (line_height - font_size)
    # --- 4. 绘制半透明背景 ---
    if bg_color and len(bg_color) == 4:
        overlay = image.copy()
        cv2.rectangle(overlay, (x, y), (x + legend_width, y + legend_height), bg_color[:3], -1)
        # 应用透明度
        alpha = bg_color[3]
        cv2.addWeighted(overlay, alpha, image, 1 - alpha, 0, image)
    # --- 5. 循环绘制每一行 ---
    for i, (name, shape, color) in enumerate(legend_items):
        # 计算当前行的垂直中心位置
        ...
        # (这里是您之前的形状绘制逻辑)
        if shape == 'circle':
            cv2.circle(image, center, icon_radius - 1, color, -1, cv2.LINE_AA)
            cv2.circle(image, center, icon_radius - 1, (0, 0, 0), 1, cv2.LINE_AA)
        elif shape == 'square':
            pt1 = (center[0] - (icon_radius-1), center[1] - (icon_radius-1))
            pt2 = (center[0] + (icon_radius-1), center[1] + (icon_radius-1))
            cv2.rectangle(image, pt1, pt2, color, -1, cv2.LINE_AA)
            cv2.rectangle(image, pt1, pt2, (0, 0, 0), 1, cv2.LINE_AA)
        elif shape == 'triangle':
            pts = np.array([
                [center[0], center[1] - icon_radius], 
                [center[0] - int(icon_radius * 0.866), center[1] + int(icon_radius * 0.5)], 
                [center[0] + int(icon_radius * 0.866), center[1] + int(icon_radius * 0.5)]
            ], np.int32)
            cv2.drawContours(image, [pts], 0, color, -1, cv2.LINE_AA)
            cv2.drawContours(image, [pts], 0, (0, 0, 0), 1, cv2.LINE_AA)
        elif shape == 'diamond':
            pts = np.array([
                [center[0], center[1] - (icon_radius-1)], [center[0] + (icon_radius-1), center[1]],
                [center[0], center[1] + (icon_radius-1)], [center[0] - (icon_radius-1), center[1]]
            ], np.int32)
            cv2.drawContours(image, [pts], 0, color, -1, cv2.LINE_AA)
            cv2.drawContours(image, [pts], 0, (0, 0, 0), 1, cv2.LINE_AA)
        elif shape == 'star':
            outer_r, inner_r = icon_radius, int(icon_radius * 0.4)
            pts = []
            for j in range(10):
            ...
            cv2.drawContours(image, [pts], 0, (0, 0, 0), 1, cv2.LINE_AA)
        # 固定位置绘制文本
        # 计算文本的左上角Y坐标，使其在垂直方向上与图标中心对齐
        text_y_pos = current_y_center - int(font_size / 2) - 2 # 微调-2像素
        text_position = (text_start_x, text_y_pos)
        
        image = putText_pil(image, name, text_position, font_path, font_size, (255, 255, 255))
...
def draw_points_on_image_multimethod(image, multi_method_xy, multi_method_error, method_styles, point_radius=12):
    ...
    for method_name, xy_dict in multi_method_xy.items():
        ...
        if method_name == 'GT':
            noise_x = np.random.uniform(0, 0.5) 
            noise_y = np.random.uniform(0, 0.5) 
            x, y = fpv_xy[0] + noise_x, fpv_xy[1] + noise_y
        # 绘制点（根据方法赋予不同形状）
        if shape == 'circle':
