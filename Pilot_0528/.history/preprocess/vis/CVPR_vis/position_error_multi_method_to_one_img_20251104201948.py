...
def draw_legend(image, legend_items, top_left_corner, bg_color=(0, 0, 0, 0.5)):
    ...
    for i, (name, shape, color) in enumerate(legend_items):
        ...
        # (形状绘制逻辑保持不变)
        if shape == 'circle':
            cv2.circle(image, center, icon_radius - 1, color, -1, cv2.LINE_AA); cv2.circle(image, center, icon_radius - 1, (0, 0, 0), 1, cv2.LINE_AA)
        elif shape == 'square':
            pt1 = (center[0] - (icon_radius-1), center[1] - (icon_radius-1)); pt2 = (center[0] + (icon_radius-1), center[1] + (icon_radius-1))
            cv2.rectangle(image, pt1, pt2, color, -1, cv2.LINE_AA); cv2.rectangle(image, pt1, pt2, (0, 0, 0), 1, cv2.LINE_AA)
        elif shape == 'triangle':
            ...
            cv2.drawContours(image, [pts], 0, color, -1, cv2.LINE_AA); cv2.drawContours(image, [pts], 0, (0, 0, 0), 1, cv2.LINE_AA)
        elif shape == 'diamond':
