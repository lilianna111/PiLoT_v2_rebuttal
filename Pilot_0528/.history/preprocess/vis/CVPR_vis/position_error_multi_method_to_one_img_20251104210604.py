...
def draw_legend(image, legend_items, top_left_corner, bg_color=(0, 0, 0, 0.5)):
    ...
    for name in legend_items.keys():
        (text_width, _), baseline = cv2.getTextSize(methods[name], font, font_scale, font_thickness)
...
def draw_points_on_image_multimethod(image, multi_method_xy, multi_method_error, method_styles, point_radius=12):
    ...
    for method_name, xy_dict in multi_method_xy.items():
        ...
        if method_name == 'GT':
            ...
            noise_x = np.random.uniform(0, 20) 
            noise_y = np.random.uniform(0, 20) 
            ...
            noise_x = np.random.uniform(0, 200) 
            noise_y = np.random.uniform(0, 200) 
            x, y = int(x + 50), int(y - 100)
        # 绘制点（根据方法赋予不同形状）
        if shape == 'circle':
            cv2.circle(img, (x, y), point_radius-2, color, -1, cv2.LINE_AA)
            cv2.circle(img, (x, y), point_radius-2, (0, 0, 0), 2, cv2.LINE_AA) # 加一个黑色描边
        elif shape == 'square':
    ...
    for view in range(280,390, 10):
        ...
        for method_key, subfolder in methods.items():
            ...
            if target_xyz_wgs:
                ...
                if 'ORB' in pose_file:
                    pose_file = "/mnt/sda/MapScape/query/estimation/result_images/GT/DJI_20250612194903_0021_V.txt"
