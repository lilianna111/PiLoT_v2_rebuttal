...
def visualize_matches(img1, img2, points1, points2, 
                      radius=8, thickness=-1, draw_lines=True, 
                      draw_indices=True,vis_save_path = None):
    """
    ...
    if vis_save_path is not None:
        save_name = os.path.join(vis_save_path, str(len(os.listdir(save_path))+1)+'.png')
...
def osg_to_ue(euler_enu):
    # 1. 根据 OSG 角反推中间提取角（与 as_euler('yxz') 对应）
    roll_osg, pitch_osg, yaw_osg = euler_enu
    pitch_extracted = pitch_osg - 90
    roll_extracted = roll_osg
    yaw_extracted   = 270 - yaw_osg

    # 2. 构造旋转矩阵（注意这里使用的顺序与原来提取时一致）
    R_osg = R.from_euler('yxz', [pitch_extracted, roll_extracted, yaw_extracted], degrees=True).as_matrix()

    # 3. 从旋转矩阵中提取 UE 下用于构造矩阵的中间角，采用 'ZYX' 顺序
    [yaw_ue_inter, pitch_ue_inter, roll_ue_inter] = R.from_matrix(R_osg).as_euler('ZYX', degrees=True)

    # 4. 反算得到 UE 欧拉角
    ue_pitch = pitch_ue_inter + 90
    ue_yaw   = 270 - yaw_ue_inter
    ue_roll  = roll_ue_inter
    return [ue_roll, ue_pitch, ue_yaw]
def ue_to_osg(euler_enu):
    roll, pitch, yaw = euler_enu
    pitch_ue = pitch - 90
    yaw_ue = 270 - yaw
    roll_ue = roll

    # 绕世界Z轴的 yaw
    euler_ue = [yaw_ue, pitch_ue, roll_ue]
    R_ue = R.from_euler('ZYX', euler_ue, degrees=True).as_matrix()

    # 从复合矩阵中提取等效的欧拉角
    # 注意：as_euler('zyx') 返回的顺序为 [roll_eq, pitch_eq, yaw_eq]
    euler_ue = R.from_matrix(R_ue).as_euler('yxz', degrees=True) # [ROLL, PITCH, YAW]
    # yxz
    yaw_osg = 270 - euler_ue[2]
    pitch_osg = euler_ue[0] + 90
    roll_osg = euler_ue[1]
    return [roll_osg, pitch_osg, yaw_osg]
...
def colmap_to_osg_bak(T_colmap_in_ned):
    ...
    return euler_angles, t_c2w, T_ECEF, T_ECEF_W2C
def get_matrix(translation, euler_angles, origin=[0,0,0], mode='c2w'):
    ...
    T_c2w[:3, :3] = R_c2w
    T_c2w[:3, 3] = t_c2w
    T_c2w[:3, 3] -= origin  # 对单个矩阵的平移部分减去 origin
    T_c2w[:3, 1] = -T_c2w[:3, 1]  # Y 轴取反
    T_c2w[:3, 2] = -T_c2w[:3, 2]  # Z 轴取反
    if mode == 'c2w':
        return T_c2w
    else:
...
# ECEF (EPSG:4978) -> WGS84经纬高 (EPSG:4326)
_transformer_ecef2wgs84 = pyproj.Transformer.from_crs(
    "EPSG:4978",  # ECEF
    "EPSG:4326",  # lon, lat, h (degrees, meters)
    always_xy=True,
)
def ECEF_to_WGS84(pos):
    """
    pos: shape (3,) or (N,3) array-like of [X, Y, Z] in meters (ECEF).
    return:
        if input (3,):  (lon, lat, h)
        if input (N,3): array of shape (N,3) with columns [lon, lat, h]
    """
    ...
    if pos.ndim == 1 and pos.shape[0] == 3:
        x, y, z = pos
        lon, lat, h = _transformer_ecef2wgs84.transform(x, y, z)
        return lon, lat, h

    # 批量 (N,3)
    assert pos.ndim == 2 and pos.shape[1] == 3, "pos must be (3,) or (N,3)"
...
def wgs84tocgcs2000_batch(trans, exp_place):
    """
    Batch convert WGS84 coordinates to CGCS2000.

    Args:
        trans (array-like): (n, 3) array of [lon, lat, height] in WGS84.
        exp_place (int): EPSG code for CGCS2000 zone (e.g. 4490, 4547...).

    Returns:
        np.ndarray: (n, 3) array of [x, y, height] in CGCS2000.
    """
    trans = np.asarray(trans)  # Ensure input is an array
    assert trans.shape[1] == 3, "Input must be (n, 3) array for [lon, lat, height]"

    wgs84 = pyproj.CRS('EPSG:4326')
    cgcs2000 = pyproj.CRS(f"EPSG:{exp_place}")
    transformer = pyproj.Transformer.from_crs(wgs84, cgcs2000, always_xy=True)
    ...
    x, y = transformer.transform(lon, lat)
    out = np.stack([x, y, height], axis=1)
