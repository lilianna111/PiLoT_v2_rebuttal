...
for seq in seq_list:
    ...
    fig, ax = plt.subplots(figsize=(4, 8)) # 稍微增大图像尺寸以容纳坐标轴
    ax.set_aspect('equal')                 # 保持xy比例
    # (新修改) 添加坐标轴标签和网格
    ax.set_xlabel("North (m)", fontsize=10)
    ax.set_ylabel("East (m)", fontsize=10)
    
    # ax.set_title(f"Trajectory: {methods_name[method]} on {seq_name}", fontsize=16)
    # ax.grid(True, linestyle='--', alpha=0.6)
    ax.set_title(f"Combined Trajectories on {seq_name}", fontsize=16) 
    ax.grid(True, linestyle='--', alpha=0.6)
    origin = poses_gt[0]
    poses_gt_origin  = poses_gt-origin
    for method, color in methods.items():
        ...
        rel[valid_mask] -= origin
        # dx, dy = offset_dict.get(method, (0.0, 0.0))
        # rel[valid_mask, 0] += dx
        # rel[valid_mask, 1] += dy
        if method != "GT":
            scale, R, t = umeyama_alignment(rel[valid_mask], poses_gt_origin[valid_mask])
        ...
        if method in ["GT", "FPVLoc"]:
            layer_order = 10  # 较高的值，确保在顶层
            line_width = 2.5  # 也可以让这两条线更粗一些以突出显示
        else:
            layer_order = 1   # 其他方法使用较低的默认值
            line_width = 2.0  # 其他线可以细一些
        # 2. 在 plot 函数中使用 zorder 和动态线宽
        ax.plot(
            rel_smoothed[valid, 1], 
            rel_smoothed[valid, 0], 
            color=color, 
            linewidth=line_width, # 使用上面设定的线宽
            alpha=0.9,
            label=methods_name[method],
            zorder=layer_order    # <-- 关键参数在这里！
        )
    #     ax.plot(
    #     rel_smoothed[valid, 1], 
    #     rel_smoothed[valid, 0], 
    #     color=color, 
    #     linewidth=2.0,          # 从 2.5 减小到 2.0 或 1.8
    #     alpha=0.9,              # 增加一点透明度 (1.0为不透明)
    #     label=methods_name[method]
    # )
    ax.legend(
    loc='lower left',      # 定位在左下角
    fontsize='small',      # 使用较小的字体
    frameon=False,         # 不显示边框
    # 或者，如果你想要边框，但希望它半透明：
    # frameon=True,
    # framealpha=0.7       # 设置背景半透明
    )
    # 保存图像
    save_path = os.path.join(outputs, f"{seq_name}_combined.png")
    fig.savefig(save_path, dpi=300, bbox_inches='tight', pad_inches=0.1) # 建议 pad_inches 留一点点，防止图例被裁
    print(f"✅ Combined plot saved: {save_path}")
    plt.close(fig)
print("✅ 每个方法的纯轨迹图已分别保存。") # 这句可以改成 "✅ 叠加图已保存。"
