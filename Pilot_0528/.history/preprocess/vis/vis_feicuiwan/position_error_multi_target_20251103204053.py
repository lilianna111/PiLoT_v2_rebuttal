...
def main():
    ...
    for txt_file in txt_files:
        ...
        for method_name, subfolder in methods.items():
            ...
            gt_xy_list = get_es_xyz(gt_pose_file)
            xy_dict = {}
            ...
            #     xy_dict[name] = xy
            for name, gt_xyz in gt_xy_list.items() :
                if name not in 'marcopolo_ideale_770' or 'Jeep_Wrangler__Mk1___YJ__1988_black': 
                    continue
                if name not in xyz_list.keys():
                    continue
                    xyz = gt_xy_list[name]
                    xyz = WGS84_to_ECEF(xyz)
                    xyz = np.expand_dims(xyz, axis = 0)
