...
def evaluate_xyz(results, gt, only_localized=False):
    ...
    with open(results, 'r') as f:
        for line in f:
            ...
            try:
                ...
                if car_name not in predictions.keys():
                    predictions[car_name] = []
                img_name = tokens[0].split('/')[-1]
                predictions[car_name][img_name] = t_ecef
    ...
    with open(gt, 'r') as f:
        for line in f:
            ...
            try:
                ...
                if car_name not in gts.keys():
                    gts[car_name] = []
                    test_names.append(car_name)
                img_name = tokens[0].split('/')[-1]
                gts[car_name][img_name] = t_ecef
                total_num += 1
    ...
    for name in test_names:
        for img_name in gts[name].keys():
            if name not in predictions or name not in gts:
                if only_localized:
                    continue
                errors_t[name] = np.inf
            else:
                t_gt = np.array(gts[name])
        ...
        for line in file:
            ...
            if parts:  # Ensure the line is not empty
                ...
                if car_name not in xyz_dict.keys():
                    xyz_dict[car_name] = []
                xyz_dict[car_name].append([x, y, z]) # Add the first element to the name list
                # xyz_dict[parts[0]] = [x, y, z] # Add the first element to the name list
    return xyz_dict 
...
def main():
    ...
    for txt_file in txt_files:
        ...
        for method_name, subfolder in methods.items():
            ...
            vis_img = draw_error_points_on_image(image, es_xy_dict, error_t)
