...
import csv
import cv2
from transform import WGS84_to_ECEF, WGS84_to_ECEF_tensor, get_matrix
...
def main():
    ...
    results_list = []
    gt_path = "/mnt/sda/MapScape/query/bbox/"
    render2orb_path = os.path.join(base_path, "FPVLoc")
    ...
    for txt_file in txt_files:
        ...
        gt_file = os.path.join(gt_path, seq,f"{seq}_RTK.txt")
        ...
        if image is None:
            ...
            continue
        gt_pose_file = os.path.join(base_path, 'GT', f"{seq}.txt")
        ...
        for method_name, subfolder in methods.items():
            ...
            for name, _ in xyz_list.items():
                if 'marcopolo_ideale_770' not in name and 'Jeep_Wrangler__Mk1___YJ__1988_black' not in name: 
                    continue
                ...
                for img_name, xyz in gt_xyz_list.items():
                    ...
                    if name not in gt_xy_dict:
                        gt_xy_dict[name] = {}
                    gt_xy_dict[name][img_name] = xy[0]
                es_xy = [es_xy_dict[name][img_name] for img_name in es_xy_dict[name].keys()]
                error = [error_t_dict[name][img_name] for img_name in error_t_dict[name].keys()]
                vis_img = draw_error_points_on_image(image, es_xy, error)
                print(f"== target : {name} ==")
