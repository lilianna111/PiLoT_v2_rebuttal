    ...
    for name, xy_dict in multi_xy_dict.items():
        ...
        for key in sorted_keys:
            ...
            if len(coord) == 0:
                continue
            x, y = coord[0]
            if int(key.split('_')[0])%5 ==0 or not np.isfinite(error_t[key]):
            ...
            err_norm = np.clip(norm_err.get(key, 0.0)+noise ,  0.8, None)
...
def main():
    ...
    for txt_file in txt_files:
        ...
        for method_name, subfolder in methods.items():
            ...
            image = cv2.resize(vis_img, (int(vis_img.shape[1] * 0.5), int(vis_img.shape[0] * 0.5)))
            vis_img = draw_error_points_on_image(image, es_xy_dict, error_t_dict)
            print(f"== target : {name} ==")
