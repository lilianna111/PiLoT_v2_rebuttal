...
# ========== 可编辑区域 ==========
FILES = [
    "/media/ubuntu/PS20001/poses/USA_seq2@8@sunny@screen16@500.txt",  # ← 最后一个：RTK-IMU GT
    "/mnt/sda/MapScape/query/estimation/result_images/Google/GeoPixel/USA_seq2@8@sunny@500.txt",  # 1612
    "/mnt/sda/MapScape/query/estimation/result_images/Google/GeoPixel/USA_seq2@8@cloudy@500.txt", #1602
    "/mnt/sda/MapScape/query/estimation/result_images/Google/GeoPixel/USA_seq2@8@foggy@intensity2@500.txt",  #1589
    "/mnt/sda/MapScape/query/estimation/result_images/Google/GeoPixel/USA_seq2@8@night@intensity2@500.txt",  #1612
    "/mnt/sda/MapScape/query/estimation/result_images/Google/GeoPixel/USA_seq2@8@sunset@500.txt",  #1598
    # "/mnt/sda/MapScape/query/estimation/result_images/Google/GeoPixels/rain.txt",  #1605
    
]
SAVE_DIR = "/mnt/sda/MapScape/query/estimation/vis/"
SAVE_NAME = "overlay_5panels.png"
USE_ID_UNION = True
...
def main(file_paths: List[str],
    ...
    for i, (xy, label, color, lw, a_) in enumerate(zip(YAWs, colors, line_widths, alphas)):
        v = ~np.isnan(h)
        if not v.any():
            continue
    ...
    for i, (h, color, lw, a_) in enumerate(zip(YAWs, colors, line_widths, alphas)):
        v = ~np.isnan(h)
        if not v.any():
            continue
    ...
    for i, (p, color, lw, a_) in enumerate(zip(YAWs, colors, line_widths, alphas)):
        v = ~np.isnan(p)
        if not v.any():
            continue
