...
# ========== 可编辑区域 ==========
FILES = [
    "/media/ubuntu/PS20001/poses/USA_seq2@8@sunny@200.txt",  # ← 最后一个：RTK-IMU GT
    "/mnt/sda/MapScape/query/estimation/result_images/Google/GeoPixel/USA_seq2@8@sunny@200.txt",  # 1612
    "/mnt/sda/MapScape/query/estimation/result_images/Google/GeoPixel/USA_seq2@8@cloudy@200.txt", #1602
    "/mnt/sda/MapScape/query/estimation/result_images/Google/GeoPixel/USA_seq2@8@foggy@intensity3@200.txt",  #1589
    "/mnt/sda/MapScape/query/estimation/result_images/Google/GeoPixel/USA_seq2@8@night@intensity3@200.txt",  #1612
    "/mnt/sda/MapScape/query/estimation/result_images/Google/GeoPixel/USA_seq2@8@sunset@200.txt",  #1598
...
def main(file_paths: List[str],
    ...
    for i, (xy, label, color, offset, lw, a_) in enumerate(zip(XYs, labels, colors, offsets, line_widths, alphas)):
        ...
        # 提取天气
        parts = label.split('@')
        label_short = parts[2] if len(parts) >= 3 else label

        # 虚线样式
        dash_style = dash_patterns[i % len(dash_patterns)]

        # 绘制轨迹
        ax_xy.plot(xx, yy, color=color, linewidth=lw, linestyle=dash_style, alpha=a_)

        # 起终点标记
        ax_xy.scatter(xx[0], yy[0], s=14, facecolor='none', edgecolor=color, linewidth=1.0, zorder=5)
        ax_xy.scatter(xx[-1], yy[-1], s=14, facecolor=color, edgecolor='none', zorder=5)

        # 创建 legend 句柄（保证线段长度一致）
        line_for_legend = mlines.Line2D([], [], color=color, linestyle=dash_style, linewidth=2.0)
        lines_for_legend.append(line_for_legend)
    ...
    # 设置正方形比例
        ...
        ax_xy.legend(
            ...
            frameon=False,
            fontsize=9,
            handlelength=3.0,    # 统一长度
            handletextpad=0.8,   # 线段与文字间距
            labelspacing=0.4
        )
