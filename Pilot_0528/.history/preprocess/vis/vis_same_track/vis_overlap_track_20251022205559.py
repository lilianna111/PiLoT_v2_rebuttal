...
def main(file_paths: List[str],
    ...
    # 首先画轨迹线，不带label
    for i, (xy, color, offset, lw, a_) in enumerate(zip(XYs, colors, offsets, line_widths, alphas)):
        v = ~np.isnan(xy[:, 0])
        if not v.any():
            continue
    ...
    # 再单独画label，用空数据点或者最后一点作为位置
    for i, (xy, label) in enumerate(zip(XYs, labels)):
        parts = label.split('@')
        if len(parts) >= 3:
            label_short = parts[2]  # 第3个字段是天气
        else:
            label_short = label
        ...
        # 使用轨迹最后一点作为 label 的位置
        xx = xy[v, 0][-1] + offsets[i][0]
        yy = xy[v, 1][-1] + offsets[i][1]
