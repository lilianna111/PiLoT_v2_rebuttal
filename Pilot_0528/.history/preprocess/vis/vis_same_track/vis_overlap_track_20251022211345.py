...
def main(file_paths: List[str],
    ...
    for i, (xy, label, color, offset, lw, a_) in enumerate(zip(XYs, labels, colors, offsets, line_widths, alphas)):
        ...
        for label in labels:
            parts = label.split('@')
            if len(parts) >= 3:
                label_shorts.append(parts[2])
            else:
                label_shorts.append(label)

        # 修改第一个
        label_shorts[0] = "RTK-IMU GT"
        ...
        # 创建一个不可见的点，只用于显示图例
        for xy, color, label_short in zip(XYs, colors, label_shorts):
            ax_xy.plot(xy[:, 0], xy[:, 1], color=color, label=label_short)
