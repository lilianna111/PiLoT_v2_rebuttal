        ...
        with open(file_path, "r") as f:
            for line in f:
                ...
                try:
                    ...
                    roll, pit, ya  = map(float, parts[4:7])
                except Exception:
                    continue
    ...
    # ---------- 情况 2: JSON ----------
        ...
        if isinstance(data, dict):
            ...
            # b) 或者直接是 list
                ...
                # 有时 estimated_poses.json 本身就是列表
        ...
        for item in entries:
            ...
            if "frame_id" in item:
                ...
                fid = parse_frame_id(str(item["name"]))
            if fid is None:
                continue
            ...
            alt = item.get("alt") or item.get("altitude") or item.get("height")
            ...
            # 姿态角
            pit = item.get("pitch") or 0.0
            ya  = item.get("yaw") or 0.0

            if ya < 0:
                ya += 360.0

            ids.append(fid)
            lonlatalt.append((float(lon), float(lat), float(alt)))
            height.append(float(alt))
