...
def load_pose_with_ids_extended(file_path: str):
    """
    仅用于读取 JSON 格式的轨迹文件
    支持 sampleinfos_interpolated.json / estimated_poses.json / 通用结构
    返回：
      ids: (N,)
      xyz: (N,3)  投影到 CGCS2000 / EPSG:4547
      height: (N,)
      pitch: (N,)
      yaw: (N,)
    """
    ...
    if isinstance(data, dict) and "samples" in data:
        for s in data["samples"]:
            name = str(s.get("name", ""))
            fid = int(re.findall(r"(\d+)", name)[0]) if re.findall(r"(\d+)", name) else len(ids)
            lon, lat, alt = s["lon"], s["lat"], s["alt"]
            pit, ya = s.get("pitch", 0), s.get("yaw", 0)
            if ya < 0:
                ya += 360
            ids.append(fid)
    ...
    # --- case 2: {"poses": {...}} ---
        ...
        for k, p in data["poses"].items():
            fid = int(re.findall(r"(\d+)", k)[0]) if re.findall(r"(\d+)", k) else len(ids)
            lon, lat, alt = p["lon"], p["lat"], p["alt"]
            pit, ya = p.get("pitch", 0), p.get("yaw", 0)
            if ya < 0:
                ya += 360
            ids.append(fid)
            lonlatalt.append((lon, lat, alt))
    ...
    # --- case 3: 直接是列表 ---
        ...
        print(f"⚠️ 未识别 JSON 格式: {file_path}")
        return np.zeros(0,), np.zeros((0,3)), np.zeros(0,), np.zeros(0,), np.zeros(0,)

    if not ids:
        print(f"⚠️ 空 JSON: {file_path}")
        return np.zeros(0,), np.zeros((0,3)), np.zeros(0,), np.zeros(0,), np.zeros(0,)

    ids = np.asarray(ids, dtype=int)
    lonlatalt = np.asarray(lonlatalt, dtype=float)
