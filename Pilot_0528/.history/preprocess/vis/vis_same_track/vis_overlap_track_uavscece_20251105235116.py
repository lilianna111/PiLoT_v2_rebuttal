...
def load_pose_from_json(file_path: str):
    ...
    for item in data:
        name = item.get("OriginalImageName", "")
        fid = parse_frame_id(name)  # ✅ 用图片名解析帧号
        if fid is None:
            continue

        lon, lat, alt = float(item['lon']), float(item['lat']), float(item['alt'])
        pit, ya = float(item['pitch']), float(item['yaw'])
    ...
    xyz = wgs84tocgcs2000_batch(lonlatalt, 4547)
