...
# === 样式设定：颜色越深表示误差越小（内圈） ===
colors = {
    '1m': '#8C3B2E',   # 深 → Recall@1m
    '3m': '#E69D8B',   # 中
    '5m': '#F7C9BD',   # 浅 → Recall@5m
}
alphas = {'1m': 0.35, '3m': 0.25, '5m': 0.15}
markers = {'1m': 'o', '3m': 's', '5m': 'D'}
marker_size = 40
z_orders = {'5m': 1, '3m': 2, '1m': 3}  # 内圈最后绘制
...
for idx, row in df.iterrows():
    ...
    for level in ['5m', '3m', '1m']:  # 外 → 内
        ...
        for ang, val in zip(angles, data):
            ax.scatter(ang, val, color=colors[level], marker=markers[level],
                       s=marker_size, zorder=z + 0.2)
    ...
    print(f"Saved to {os.path.join(output_dir, f'{method}_radar.png')}")
    plt.close()
