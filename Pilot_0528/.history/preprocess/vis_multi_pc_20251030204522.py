...
import numpy as np
import torch
from transform import get_matrix_list_batch
...
pose_file = "/media/ubuntu/PS20001/poses/USA_seq5@8@cloudy@300-100@200"
...
intrinsics = np.array([[1931.7, 0, 800],[0, 1931.7, 600], [0, 0, 1]])

# Get all image files from multi directory
import os
import glob

multi_dir = r"C:\Users\74578\Desktop\Data"
...
for i, (img_file, depth_file) in enumerate(zip(image_files, depth_files)):
    ...
    depthr = cv2.imread(depth_file, cv2.IMREAD_UNCHANGED)
    ...
    img_num = os.path.basename(img_file).split('_')[0]
    if str(img_num) in pose_dict.item():
        query_pose = pose_dict.item()[str(img_num)]
    ...
    if i == 0:
        ...
        all_poses_array = np.array([all_pose.item()[str(j)] for j in range(len(image_files)) if str(j) in all_pose.item()])
