...
from transform import ECEF_to_WGS84, ecef_to_gausskruger_pyproj,qvec2rotmat
...
def quick_scatter3d_minimal(points_xyz: np.ndarray,
    ...
    """
    透明背景、极简黑色XYZ轴、带三色三轴小图标的点云可视化。
    - points_xyz: [N,3]
    - colors_rgb: [N,3] in 0..255 (optional)
    - save_path: 若给定则保存PNG/SVG等（使用透明背景）
    """
    if points_xyz.shape[0] == 0:
        print("[info] no valid 3D points to visualize.")
        return
    ...
    # 也可选择去掉所有刻度：
    # ax.set_xticks([]); ax.set_yticks([]); ax.set_zhicks([])
    ...
    # —— 右下角三色三轴“小图标”（2D inset）——
    # 用二维小绘图区，在屏幕坐标系画三条箭头
    inset = fig.add_axes([0.83, 0.08, 0.12, 0.12])  # [left, bottom, width, height] 相对坐标
    ...
    # 画箭头：X=红、Y=绿、Z=蓝
    # 用2D表示Z轴，带轻微斜向效果
    arrow_kw = dict(width=0.012, head_width=0.05, head_length=0.06, length_includes_head=True)
    inset.annotate('', xy=o + [L, 0], xytext=o, arrowprops=dict(color='#E53935', **arrow_kw))  # X (red)
    inset.text(*(o + [L+0.05, 0]), 'X', color='#E53935', fontsize=9, va='center')
    inset.annotate('', xy=o + [0, L], xytext=o, arrowprops=dict(color='#43A047', **arrow_kw))  # Y (green)
    inset.text(*(o + [0, L+0.05]), 'Y', color='#43A047', fontsize=9, ha='center')
    inset.annotate('', xy=o + [L*0.6, L*0.6], xytext=o, arrowprops=dict(color='#1E88E5', **arrow_kw))  # Z (blue)
    inset.text(*(o + [L*0.6+0.05, L*0.6+0.02]), 'Z', color='#1E88E5', fontsize=9)

    plt.tight_layout()

    if save_path:
        # 透明背景保存；如需白底可改 transparent=False, facecolor='white'
