...
def quick_scatter3d_minimal(points_xyz: np.ndarray,
                            ...
                            axes_style: str = "corner"):  # "corner" | "none"
    if points_xyz.shape[0] == 0:
        print("[info] no valid 3D points to visualize.")
        return
    ...
    # 点云
    ax.scatter(P[:, 0], P[:, 1], P[:, 2], s=0.2, c=C, depthshade=False)
    ...
    # —— 黑色轴线：画在角落（不穿过点云）或完全不画 ——
    if axes_style == "corner":
        # 选包围盒靠“后-左-下”的角作为原点
        ox, oy, oz = center[0]-r*0.95, center[1]-r*0.95, center[2]-r*0.95
        L = r * 0.45  # 轴长度
        lw = 1.0
        ax.add_line(Line3D([ox, ox+L], [oy, oy],   [oz, oz],   color='black', linewidth=lw))  # X
        ax.add_line(Line3D([ox, ox],   [oy, oy+L], [oz, oz],   color='black', linewidth=lw))  # Y
        ax.add_line(Line3D([ox, ox],   [oy, oy],   [oz, oz+L], color='black', linewidth=lw))  # Z
        ax.text(ox+L, oy,   oz,   'X', color='black', fontsize=9)
        ax.text(ox,   oy+L, oz,   'Y', color='black', fontsize=9)
        ax.text(ox,   oy,   oz+L, 'Z', color='black', fontsize=9)
    elif axes_style == "none":
        # 去掉刻度标签与刻度线，完全干净
        ax.set_xticks([]); ax.set_yticks([]); ax.set_zticks([])
    else:
        raise ValueError("axes_style must be 'corner' or 'none'")

    # —— 右下角三色三轴小图标（2D inset；用 Axes.arrow）——
    inset = fig.add_axes([0.83, 0.08, 0.12, 0.12])
    inset.set_facecolor('none')
    inset.set_xlim(0, 1); inset.set_ylim(0, 1)
    inset.axis('off')
    o = np.array([0.15, 0.15]); L2 = 0.65
    # X=红
    inset.arrow(o[0], o[1], L2, 0, width=0.012, head_width=0.06, head_length=0.08,
                length_includes_head=True, color='#E53935')
    inset.text(o[0] + L2 + 0.05, o[1], 'X', color='#E53935', fontsize=9, va='center')
    # Y=绿
    inset.arrow(o[0], o[1], 0, L2, width=0.012, head_width=0.06, head_length=0.08,
                length_includes_head=True, color='#43A047')
    inset.text(o[0], o[1] + L2 + 0.05, 'Y', color='#43A047', fontsize=9, ha='center')
    # Z=蓝（斜向）
    inset.arrow(o[0], o[1], L2*0.6, L2*0.6, width=0.012, head_width=0.06, head_length=0.08,
                length_includes_head=True, color='#1E88E5')
    inset.text(o[0] + L2*0.6 + 0.05, o[1] + L2*0.6 + 0.02, 'Z', color='#1E88E5', fontsize=9)
...
if __name__=="__main__":
    ...
    quick_scatter3d_minimal(pts_ecef, cols, max_points=200000,
