...
class RenderImageProcessor:
    ...
    def rendering(self, config_web):
        ...
        # if '.' not in self.image_id:
        self.image_id = self.image_id.split('.')[0] + '.png'
        self.outputs = Path(config_web['outputs'])
...
if __name__ == "__main__":
    ...
    config = {
        ...
        #     4.71 # sensor_height
        # ],
        "output":"/mnt/sda/MapScape/query/estimation/mp4_compare/USA_seq5_foggy_gif/PixLoc"
    }
    ...
    method_list = ["GeoPixel", "PixLoc", "Render2Loc", "Render2ORB", "Render2RAFT"]
    method_list = ["Render2ORB"]
    
    pose_save_path = "/mnt/sda/MapScape/query/estimation/mp4_compare/USA_seq5_foggy_gif/PixLoc/USA_seq5@8@foggy@200.txt"
    
    for method in method_list:
        pose_save_path = os.path.join('/mnt/sda/MapScape/query/estimation/mp4_compare/USA_seq5_foggy_gif', method, 'USA_seq5@8@foggy@200.txt')
        # load map 
        config_web['outputs'] =  os.path.join('/mnt/sda/MapScape/query/estimation/mp4_compare/USA_seq5_foggy_gif/large3840')
        with open(pose_save_path, 'r') as f:
            for data in f.read().rstrip().split('\n'):
                ...
                config_web['image_id'] =  name.split('.')[0]+'_' +method
        ...
        with open(pose_save_path, 'r') as f:
            for data in tqdm(f.read().rstrip().split('\n')):
                tokens = data.split()
                name = os.path.basename(tokens[0])
                if int(name.split('_')[0]) < start or int(name.split('_')[0]) > end:
                    continue
                # Split the data into the quaternion and translation parts
                t_c2w, e_c2w  = np.split(np.array(tokens[1:], dtype=float), [3])
                e_c2w = [e_c2w[1], e_c2w[0], e_c2w[2]]
