...
class DualProcessTask:        
    def __init__(self, config, init_euler = None, init_trans = None, name = None):
        ...
        output_folder = "/media/amax/AE0E2AFD0E2ABE69/datasets/outputs/FPVLoc"
    ...
    def rendering_worker(self):
        ...
        #第一阶段：初始化环境 (Initialization)
        # 初始化渲染器，加载配置文件
        renderer      = osg_render.RenderImageProcessor(self.render_config)
        # 创建一个 CUDA 流，用于异步执行渲染任务，避免阻塞主线程或其他 GPU 操作
        render_stream = torch.cuda.Stream()
        torch.cuda.synchronize()    # 确保之前的 GPU 任务都完成了
        fps_log_every = 0
        idx = 0
        #第二阶段：获取任务 (Fetch Pose)
        while True:
            try:
            ...
            #第三阶段：执行渲染 (Rendering)
            # 指定在之前创建的 render_stream 中执行 GPU 操作
            ...
            # 调试功能：把渲染出来的 RGB 图存到硬盘上
            renderer.save_color_image(os.path.join(self.outputs, str(fps_log_every-2)+'_0.png'))
            
            print('render:', os.path.join(self.outputs, str(fps_log_every-2)+'_0.png'))
            # np.save(os.path.join(self.outputs, str(fps_log_every-2)+'_1.png'), depth)
            # 等待 GPU 也就是 render_stream 里的活干完，确保 color 和 depth 已经生成好了
            torch.cuda.current_stream().synchronize()
            t_render = (time.perf_counter() - t0) * 1e3  # ms

            #第四阶段：数据后处理与分发 (Post-processing & Handoff)
            # 如果网络模型要求输入尺寸必须是 16 的倍数，这里对图像进行填充
            if self.padding:
            ...
            # ---- 4) 推送给定位 ----
            try:
                # 将渲染结果 (Color, Depth) 和原始位姿 (Euler, Trans) 打包放入 task_q
                # task_q 是连接“渲染线程”和“定位线程”的管道
    ...
    def back_project(self, depth_frame, euler_angles, translation, query_euler_angles, query_translation, num_samples = 5000, device = 'cuda'):
        # 
