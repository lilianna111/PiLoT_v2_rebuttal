...
def ray_cast_to_dsm(DSM_path, K, pose, ref_npy_path, geotransform, uv, dsm_transform, num_sample=4000):
    """
    ...
    try:
        ...
        hit_point = locator.predict_center_alt(DSM_path, pose, ref_npy_path, geotransform, K, num_sample=num_sample, object_pixel_coords=[uv[0], uv[1]])
...
def crop_dsm_dom_point(DSM_path, pose, ref_npy_path, geotransform, dom_data, dsm_data, dsm_transform,
    ...
    dsm_indices = []
    for uv in image_points:
        xyz, rc = ray_cast_to_dsm(DSM_path, K, pose, ref_npy_path, geotransform, uv, dsm_transform)
        # if xyz is None:
        #     # 回退：与最小高度平面相交
        #     xyz = pixel_to_world(K, pose_w2c, uv, target_z=area_minZ)
...
def generate_ref_map(DSM_path, pose, ref_npy_path, geotransform, query_intrinsics, query_poses, name, area_minZ, dsm_data, dsm_transform, dom_data, ref_rgb_path, ref_depth_path, crop_padding=2, debug=True):
