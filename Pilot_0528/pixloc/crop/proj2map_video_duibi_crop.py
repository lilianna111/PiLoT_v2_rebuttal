...
def build_rotated_rectangle_from_center_and_edge(center_world, short_edge_start, short_edge_end):
    """
    基于中心点投影和梯形短边构建斜向长方形。

    设计约定：
    1. 矩形的一条边 = 梯形的短边（位置和长度完全一致）
    2. 矩形中心 = query 图像中心点投影到地面的世界坐标
    3. 矩形的另一条边长度由中心点到短边的垂直距离决定（距离的2倍）

    参数:
        center_world: 矩形中心点的世界坐标 (x, y, z) 或 (x, y)
        short_edge_start: 梯形短边起点的世界坐标 (x, y, z) 或 (x, y)
        short_edge_end: 梯形短边终点的世界坐标 (x, y, z) 或 (x, y)

    返回:
        rect_vertices: 长方形四个顶点的世界坐标列表 [(x1, y1), (x2, y2), (x3, y3), (x4, y4)]
    """
    center_xy = np.array(center_world[:2], dtype=np.float64)
    p_start = np.array(short_edge_start[:2], dtype=np.float64)
    p_end = np.array(short_edge_end[:2], dtype=np.float64)

    # 短边的中点
    short_edge_midpoint = (p_start + p_end) / 2.0

    # 短边方向向量（归一化）
    edge_vector = p_end - p_start
    edge_length = np.linalg.norm(edge_vector)
    if edge_length == 0:
        edge_length = 1.0
        u = np.array([1.0, 0.0], dtype=np.float64)
    else:
        u = edge_vector / edge_length

    # 垂直于短边的方向
    v = np.array([-u[1], u[0]], dtype=np.float64)

    # 从短边中点到矩形中心的向量
    center_offset = center_xy - short_edge_midpoint

    # 计算中心点到短边的垂直距离（投影到垂直方向）
    perpendicular_distance = np.dot(center_offset, v)

    # 矩形的另一条边长度 = 垂直距离的2倍
    other_edge_length = abs(perpendicular_distance) * 2.0

    # 构建矩形的四个顶点
    # 短边的两个端点
    half_u = edge_length / 2.0
    half_v = other_edge_length / 2.0
...
def generate_ref_map(query_intrinsics, query_poses, area_minZ, dsm_data, dsm_transform, dom_data, ref_rgb_path, ref_depth_path, debug, crop_vis_save_path=None, test_ratio=None, use_bbox=True):
    """
    为多个crop比例生成独立的reference文件
    每个比例会生成独立的DOM图片和点云文件
...
def crop_dsm_dom_angle(dom_data, dsm_data, dsm_transform,
    ...
    if target_ratio is None:
        ...
        if short_edge_info is None:
            ...
            print(f"短边顶点索引：起点={start_idx}, 终点={end_idx}")
            ...
            # 步骤3: 使用短边的起点和终点构建矩形
            short_edge_start = world_points[start_idx]
            short_edge_end = world_points[end_idx]
            rect_vertices = build_rotated_rectangle_from_center_and_edge(
                center_xyz, short_edge_start, short_edge_end
            )
            print(f"长方形顶点（世界坐标）:")
