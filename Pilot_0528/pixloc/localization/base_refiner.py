...
class BaseRefiner:
    ...
    def refine_query_pose(self, qname: str, qcamera: Camera, ref_camera: Camera, render_frame, T_query_initial: Pose, T_render: Pose, Points_3D_ECEF, p2d_r=None,visible_r=None,dd = None, gt_depth=None, gt_roll=None, gt_pitch=None, dsm_path=None, render_T_ecef=None,
        ...
        feature_extract_ms = (_sync_now() - feature_t0) * 1e3
        ...
        optimizer_ms = (_sync_now() - optimizer_t0) * 1e3
    ...
    def refine_pose_using_features(self,
        ...
        optimizer_level_times_ms = []
        optimizer_level_detail_ms = []
        ...
        # for idx, level in enumerate(range(len(features_query))):
            ...
            run_timing = getattr(opt, 'last_run_timing', {})
            if fail.all().item():
                return {**ret, 'success': False}

            T_i = T_opt  #!?
            # Keep the full candidate set for the coarsest level, then retain
            # only the best poses for finer levels to reduce later-stage cost.
            if idx == 0 and T_i.shape[0] > 128:
                loss_for_rank = overall_loss.clone()
                if fail.dtype != torch.bool:
                    fail = fail.bool()
                loss_for_rank = torch.where(
                    fail,
                    torch.full_like(loss_for_rank, 1e12),
                    loss_for_rank,
                )
                keep_k = min(128, loss_for_rank.shape[0])
                keep_idx = torch.topk(loss_for_rank, k=keep_k, largest=False).indices
                T_i = T_i[keep_idx]
            optimizer_level_times_ms.append(float((_sync_now() - level_t0) * 1e3))
            optimizer_level_detail_ms.append({
                'level': int(level),
                'num_iters': int(run_timing.get('num_iters', 0)),
                'optimizer_total_ms': float(run_timing.get('optimizer_total_ms', 0.0)),
                'residual_eval_ms': [float(x) for x in run_timing.get('residual_eval_ms', [])],
                'residual_cost_breakdown': list(run_timing.get('residual_cost_breakdown', [])),
                'linear_solve_ms': [float(x) for x in run_timing.get('linear_solve_ms', [])],
                'pose_update_ms': [float(x) for x in run_timing.get('pose_update_ms', [])],
                'post_cost_ms': [float(x) for x in run_timing.get('post_cost_ms', [])],
                'post_cost_breakdown': list(run_timing.get('post_cost_breakdown', [])),
                'iter_total_ms': [float(x) for x in run_timing.get('iter_total_ms', [])],
            })
            # end_time2 = time.time()
        ...
        return {
            ...
            'fail_list': fail,
            'optimizer_level_times_ms': optimizer_level_times_ms,
            'optimizer_level_detail_ms': optimizer_level_detail_ms,
        }
