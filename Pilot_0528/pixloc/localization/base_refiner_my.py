...
def build_c2w_batch(T_batch, dd, mul, origin):
    ...
    操作：
        build_c2w_batch 会执行逆操作：
        除以 mul（放大回去）。
        加上 dd（移回原点）。
        加上 origin（移回地球上的绝对位置）
    """
    # 1) Device & dtype
...
class BaseRefiner:
    ...
    def refine_query_pose(self, qname: str, qcamera: Camera, ref_camera: Camera, render_frame, T_query_initial: Pose, T_render: Pose, Points_3D_ECEF, dd = None,
                          multiscales: Optional[List[int]] = None, gt_pose_dict=None, last_frame_info = {}, query_resize_ratio = 1, image_query=None,gt_depth=None,dsm_tensor=None,dsm_meta=None) -> Dict:
        (qcamera, ref_camera, render_frame, T_query_initial, T_render, Points_3D_ECEF, dd, last_frame_info, dsm_tensor) = \
        move_inputs_to_cuda(qcamera, ref_camera, render_frame, T_query_initial, T_render, Points_3D_ECEF, dd, last_frame_info, dsm_tensor)
        ...
        ret = self.refine_pose_using_features(features_query, scales_query,
                                            ...
                                            features_ref_dense, scales_ref, p3d = Points_3D_ECEF,gt_depth=gt_depth, dsm_tensor=dsm_tensor,dsm_meta=dsm_meta)
        ...
        if ret['success']:
            ...
            # ------
            #1. 还原世界坐标 (Coordinate Restoration)
            ## 将优化后的局部坐标 (Local W2C/C2W) 还原回 ECEF/Global 坐标 (C2W)
            ...
            #2. 计算偏差 (Error Calculation)
            #计算了优化结果 (T_opt) 和先验/真值 (T_prior) 之间的距离
            dR, dt = (T_prior_ECEF_Pose.inv() @ T_opt_c2w_Pose).magnitude()
            
            #3. 严格的筛选 (Hard Filtering)
            t_indices = dt <= dis_thes #!
            R_indices = dR <= R_thes

            # 剔除旋转变化量和平移变化量过大的候选
            # 综合筛选：
            # 1. ~fail_list: 优化器本身没有报错 (比如 Hessian 矩阵可逆)
            # 2. t_indices: 位置没飘太远
            # 3. R_indices: 角度没歪太离谱
            ...
            ## 在剩下的靠谱结果里，选 Loss 最小的那个
            min_index_in_valid = torch.argmin(valid_loss)

            # ------
            #根据排名表，把那个最好的位姿找出来 (pose_index)
            pose_index = torch.nonzero(valid)[min_index_in_valid].item()
            T_refined = ret['T_opt'][pose_index]  # 提取优化后的最终位姿
            # 计算优化量：终点 (Refined) 相对于 起点 (Initial) 变了多少
            dR, dt = (T_query_initial_poses[pose_index].inv() @ T_refined).magnitude()
            ret = {
                **ret,
                'T_refined': T_refined,     #保存最好的位姿
                'diff_R': dR.item(),        # 旋转改变量
                'diff_t': dt.item(),        # 平移改变量
            }
            ...
            # 2. 取出冠军的 ECEF 世界坐标 (C2W)
            T_opt_c2w_ = T_opt_c2w[pose_index]
            # print('pose_index:',pose_index)
            # 3. 核心转换函数：PixLoc -> OSG，把pixloc算法内部的数学矩阵，翻译成osg能理解的“位置+角度”格式 (pixloc_to_osg)
    ...
    def refine_pose_using_features(self,
        ...
        # ret = self.refine_pose_using_features(features_query, scales_query,
        #                             camera, T_query_initial_poses, ref_camera, T_render,
        ...
        features_query = [feat.to(self.device) for feat in features_query] #把提取好的图像特征（Tensor 张量）从 CPU 内存 转移到 GPU 显存
        ...
        # 构建了一个 Python 字典（Dictionary），准备将计算好的初始位姿猜测传回
        ...
        #遍历特征金字塔，但是是从**顶层（最低分辨率）向底层（最高分辨率）**倒着走的
        #由粗到细
        ...
        # for idx, level in enumerate(range(len(features_query))):
            ...
            #根据当前图像金字塔的分辨率，调整相机的内参矩阵（Intrinsics）
            #图片变小了，相机参数里的“像素焦距”和“光心位置”也得跟着按比例变小
            ...
            opt = self.optimizer
            if isinstance(opt, (tuple, list)):             #tuple元组。 变量 opt 是不是一个列表或者元组？
            #从一堆数据中，精准挑出当前层级（Level）所需要的那一个
                if self.conf.layer_indices:    #layer_indices（层级索引映射表）
                    opt = opt[self.conf.layer_indices[level]]   #查表取值
                else:
                    opt = opt[level]
            if features_last_query is not None:
                ...
                last_F_query = None
                last_c_query = None
            T_opt, fail, overall_loss = opt.run(p3d, F_ref, F_q, T_i.to(F_q),   #把 T_i（初始位姿）的存储设备和数据类型，强行转换成和 F_q（查询特征）一样，GPU + float32
