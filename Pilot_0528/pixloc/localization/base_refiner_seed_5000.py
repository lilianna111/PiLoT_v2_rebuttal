...
class BaseRefiner:
    ...
    def _filter_points_by_uncertainty(self,
                                      ...
                                      top_k: int = 500):
        """
        根据置信度图筛选 3D 采样点：保留置信度最高（不确定度最低）的 top_k 个点。
        - features_ref: dense 特征（最后一通道为置信度）
        - rcamera: 渲染相机（用于把 3D 点投影回像素）
        - T_render: 渲染帧的位姿（w2c）
        - points_3d: 3D 采样点，形状 [N,3]
        """
        if (not self.conf.compute_uncertainty or
                features_ref is None or
                points_3d is None or
                T_render is None):
        ...
        # T_render 同步到置信度图所在设备
        try:
            T_render_dev = T_render.to(confidence_map)
        except Exception:
            T_render_dev = T_render

        # 3) 投影到渲染相机坐标，得到 2D 像素与可见性
        p3d_cam = T_render_dev * points_3d
        p2d_r, visible_r = rcamera.world2image(p3d_cam)
        if visible_r.dtype != torch.bool:
            visible_r = visible_r.bool()
        if not visible_r.any():
            return points_3d, confidence_map

        p2d_valid = p2d_r[visible_r]
        points_valid = points_3d[visible_r]

        # 4) 将像素坐标映射到置信度图分辨率
        scale = float(confidence_map.shape[-1]) / float(rcamera.size[0])
        coords = p2d_valid * scale
    ...
    def refine_query_pose(self, qname: str, qcamera: Camera, ref_camera: Camera, render_frame, T_query_initial: Pose, T_render: Pose, Points_3D_ECEF, dd = None,
        ...
        # 基于置信度图筛选 3D 采样点（5000 -> 500）
        if Points_3D_ECEF is not None:
            Points_3D_ECEF, _ = self._filter_points_by_uncertainty(
                features_ref_dense, ref_camera_modified, T_render, Points_3D_ECEF, top_k=500)
