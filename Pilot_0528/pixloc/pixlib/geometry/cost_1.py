...
class DirectAbsoluteCost2:
    def __init__(self):
        ...
        self.angle_target_ratio = 0.5  # 0.3→0.5：增强小残差帧的角度修正力度
        # 自适应缩放的钳制范围
        # scale_max 控制小残差帧角度的最大强度，不影响大残差帧（gate=0 时 scale 乘以 0 = 无贡献）
        self.angle_scale_min = 0.05
        self.angle_scale_max = 20.0  # 恢复原始值：gate_min=0 才是防跑飞的保障，不是 scale_max
        # 角度残差门控（单位：度）：残差越大说明pose漂移越严重，逐步减弱角度避免与重投影打架
        # <1°：全强度（追踪精准，角度起决定作用）
        # 1-8°：线性衰减（中间过渡）
        # >8°：完全关闭 → 退回纯深度稳定模式（feicuiwan 问题区段）
        self.angle_gate_start_deg = 1.0
        self.angle_gate_end_deg = 8.0
        self.angle_gate_min = 0.0    # 允许完全归零，这才是防跑飞的真正保障
        # 候选筛选不引入角度，防止扰动深度模式的收敛路径
    ...
    def residual_jacobian_batch_quat(
        ...
        # 角度与深度可同时存在
        # 关键修复：w_loss_proj 必须独立计算，不能混入 cost_angle，否则鲁棒截断阈值被污染，
        # 导致重投影权重被压低，精度下降。角度/深度各自使用独立的鲁棒权重。
        w_loss_angle = None
        if angle_res is not None:
            cost_angle = (angle_res ** 2).sum(-1, keepdim=True)  # [1, num_init_pose, 1]
            # w_loss_proj 独立计算（与 enable_angle_prior=False 时完全一致，不受角度污染）
        ...
        if angle_res is not None and w_loss_angle is not None:
            ...
            # 只加旋转-旋转子块（前3×3），不加平移和交叉项
            # 平移块不加：防止通过交叉项干扰 yaw 方向的 LM 步长
            Hess[:, :, :3, :3] = Hess[:, :, :3, :3] + Hess_angle_scaled[:, :, :3, :3]
