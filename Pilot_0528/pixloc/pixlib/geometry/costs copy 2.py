...
class DirectAbsoluteCost2:
    ...
    def residual_jacobian_batch_quat(
        ...
        # === Step 6: 深度约束 (DSM 高度残差) ===
        if (
            render_T_ecef is not None
            and gt_depth is not None
            and dsm_path is not None
            and hasattr(self, "last_wgs84_poses")
            and hasattr(self, "last_wgs84_center_points")
            and hasattr(self, "last_dsm_heights")
        ):
            depth_weight = 1e-6
            depth_truncate = 5.0
            eps_pos = 1e-6   # degrees for lon/lat
            eps_alt = 0.5    # meters
            ...
            poses_batch = []
            for i in range(num_init_pose):
                base_pose = list(self.last_wgs84_poses[i])
                poses_batch.append(base_pose)
                for j in range(6):
                    pose_pert = base_pose.copy()
                    if j == 0:
                        pose_pert[0] += eps_pos
                    elif j == 1:
                        pose_pert[1] += eps_pos
                        ...
                        pose_pert[5] += eps_ang
                    poses_batch.append(pose_pert)

            centers_batch = self._center_points_from_wgs84_poses(poses_batch, gt_depth)
            ...
            residuals_batch = centers_np[:, 2] - dsm_np
            ...
            # no depth residual thresholding
            # Drop obvious outliers by trimming the largest residuals.
            abs_res = np.abs(base_residuals)
            valid_abs = abs_res[np.isfinite(abs_res)]
            if valid_abs.size > 0:
                clip_val = np.percentile(valid_abs, 90)
                inlier_mask = (abs_res <= clip_val).astype(np.float32)
                w_loss = w_loss * torch.from_numpy(inlier_mask).to(device=grad.device, dtype=grad.dtype)

            finite_res = base_residuals[np.isfinite(base_residuals)]
            if finite_res.size > 0:
                abs_res = np.abs(finite_res)
                print(
                    "[depth-debug] abs_residual range: "
                    f"min={abs_res.min():.3f} max={abs_res.max():.3f}"
                )
            ...
            grad_depth[0] = J_depth_torch * weighted

            w_hess = (w_loss * depth_weight).view(-1, 1, 1)
            Hess_depth[0] = (
                J_depth_torch[:, :, None] * J_depth_torch[:, None, :] * w_hess
            )

            grad_norm = torch.norm(grad)
            grad_depth_norm = torch.norm(grad_depth)
            Hess_norm = torch.norm(Hess)
            Hess_depth_norm = torch.norm(Hess_depth)
            print(
                "[depth-debug] grad_ratio=%.4f Hess_ratio=%.4f"
                % (
                    (grad_depth_norm / (grad_norm + 1e-8)).item(),
                    (Hess_depth_norm / (Hess_norm + 1e-8)).item(),
                )
            )
