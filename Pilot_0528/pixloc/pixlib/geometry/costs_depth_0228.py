...
class DirectAbsoluteCost2:
    ...
    def residual_jacobian_batch_quat(
        ...
        # 角度约束先保留代码但当前默认关闭（enable_angle_prior=False）。
        # 本文件为“仅深度约束”：只启用深度 prior，角度不并入优化。
        angle_res = None
        J_angle_tensor = None
        angle_weight = 1.0

        # === Step 6: Angle prior（默认关闭）===
        # 当 enable_angle_prior=False 时，angle_res / J_angle_tensor 保持 None，仅深度约束生效。
        #
        # 角度 prior 的目标：
