...
class DirectAbsoluteCost2:
    ...
    def residual_jacobian_batch_quat(
        ...
        if (
            ...
            # === 角度约束：roll 和 pitch 的 GT 为 0（有限差分法，与参考文件一致）===
        ...
        if depth_res is not None and w_loss_depth is not None:
            ...
            if self.iter <= 3:
                ...
                print(f"[角度约束]")
                roll_base_deg = np.rad2deg(roll_base)
                pitch_base_deg = np.rad2deg(pitch_base)
                print(f"  roll残差(原始): mean={np.abs(roll_base_deg).mean():.4f}°, max={np.abs(roll_base_deg).max():.4f}°")
                print(f"  pitch残差(原始): mean={np.abs(pitch_base_deg).mean():.4f}°, max={np.abs(pitch_base_deg).max():.4f}°")
