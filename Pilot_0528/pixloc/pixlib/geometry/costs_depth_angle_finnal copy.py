...
class DirectAbsoluteCost2:
    ...
    def residual_jacobian_batch_quat(
        ...
        if (
            ...
            # GT 角度先转“度”，再做差并 wrap 到 [-180, 180]，最后转弧度作为残差
        ...
        if angle_res is not None:
            ...
            # 拼接 cost，统一计算鲁棒核权重（但返回的 cost 仍只返回投影部分）
            cost_all = torch.cat([cost_proj, cost_angle], dim=-1)  # [1, num_init_pose, N+1]
        ...
        if angle_res is not None and w_loss_angle is not None:
            ...
            angle_scale = angle_scale.clamp(max=200.0)

            grad = grad_proj + grad_angle * angle_scale
            Hess = Hess_proj + Hess_angle * angle_scale
