...
class DirectAbsoluteCost2:
    def __init__(self):
        ...
        # Guardrail penalty used only for selection: adds penalty when depth residual exceeds gate_start.
        # Larger => more likely to reject hypotheses that \"fly\" above/below DSM.
        self.depth_guard_weight = 5.0
    ...
    def residual_jacobian_batch_quat(
        ...
        if angle_res is not None:
            ...
            if depth_res is not None:
                ...
                cost = torch.cat([
                    cost_proj_robust,
                    (self.depth_guard_weight * (torch.relu(depth_abs_m_sel - self.depth_gate_start_m) / (self.depth_gate_end_m - self.depth_gate_start_m + 1e-12)).clamp(min=0.0, max=1.0)),
