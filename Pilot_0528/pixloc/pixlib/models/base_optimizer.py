...
from ..geometry.costs import DirectAbsoluteCost2
...
class BaseOptimizer(BaseModel):
    ...
    def _init(self, conf):
        ...
        self.cost_fn = DirectAbsoluteCost2()
    ...
    def _run(self, p3D: Tensor, F_ref: Tensor, F_query: Tensor,
             T_init, camera: Camera, batch = False, mask: Optional[Tensor] = None,
        ...
        if not batch:
            ...
            failed = torch.full(T.shape, False, dtype=torch.bool, device=T.device)
            for i in range(self.conf.num_iters):
                res, valid, w_unc, _, J = self.cost_fn.residual_jacobian_batch(T, *args)
                if mask is not None:
                    valid &= mask
                failed = failed | (valid.long().sum(-1) < 10)  # too few points
