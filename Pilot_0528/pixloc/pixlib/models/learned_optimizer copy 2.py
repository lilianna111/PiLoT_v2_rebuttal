...
class LearnedOptimizer(BaseOptimizer):
    ...
    def _run(self, p3D: Tensor, F_ref: Tensor, F_query: Tensor,
             ...
             gt_depth=None, dd=None, mul=None, origin=None, dsm_path=None, render_T_ecef=None):
        ...
        for i in range(num_iters): #self.conf.num_iters
            ...
            tt1 = time.time()
            print("residual_jacobian_batch_quat: ", tt1 - tt0)
            ...
            prior_cost = None
            if hasattr(self.cost_fn, "last_prior_cost"):
                prior_cost = self.cost_fn.last_prior_cost
            if prior_cost is not None:
                prior_weight = 0.05
                overall_losses.append(cost + prior_cost.unsqueeze(-1) * prior_weight)
            else:
