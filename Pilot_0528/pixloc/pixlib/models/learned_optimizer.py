...
# 计时开关：与 costs.py 共用环境变量 PILOT_SYNC_TIMING=1，开时取时间戳前先 cuda.synchronize()，
# 使分项计时落到真实 GPU 计算上（默认关闭，生产零开销，不改变任何计算结果）。
import os as _os
_SYNC_TIMING = _os.environ.get("PILOT_SYNC_TIMING", "0") == "1"

# 阶段三粗层候选预筛：优化前用 cost_only 廉价打分裁到 top-K 再做完整优化。
# 用环境变量 PILOT_PREFILTER_KEEP 控制保留数（默认 144，对齐原版候选数）；
# 设得 >= 候选总数即关闭预筛。可不改代码直接 sweep K 找”精度不变差的最小值”。
_PREFILTER_KEEP = int(_os.environ.get("PILOT_PREFILTER_KEEP", "256"))
...
class LearnedOptimizer(BaseOptimizer):
    ...
    def _run(self, p3D: Tensor, F_ref: Tensor, F_query: Tensor,
        ...
        # === 阶段三：粗层候选预筛 ===
        # 候选数远超后续保留数时，先用 cost_only 对全部候选廉价打分（无雅可比/FD/系统构建），
        # 按总 cost 升序裁到 top-K，再对 K 个候选做完整优化。排序口径与最终选择一致（cost.sum）。
        # 只裁候选维：同步重建 qcamera/args/failed（qcamera 是唯一按候选数展开的张量）。
        if num_init_pose > _PREFILTER_KEEP:
            with torch.no_grad():
                _, _, _, _, _, cost_pre = self.cost_fn.residual_jacobian_batch_quat(
                    T_flat.unsqueeze(0), *args, cost_only=True
                )
                cost_pre = cost_pre.squeeze(0) if cost_pre.shape[0] == 1 else cost_pre
                score_pre = cost_pre.sum(dim=1)  # 每候选总 cost
                keep_idx = torch.topk(score_pre, k=_PREFILTER_KEEP, largest=False).indices
            T = T[keep_idx]
            T_flat = T.to_flat()
            num_init_pose = T.shape[0]
            qcamera = camera.unsqueeze(0).expand(num_init_pose, -1)
            args = (F_ref,
                    T_render.unsqueeze(0),
                    ref_camera.unsqueeze(0),
        ...
        for i in range(num_iters): #self.conf.num_iters
            ...
            residual_t0 = _sync_now()
            g, H, w_loss, valid, p2d_q, cost = self.cost_fn.residual_jacobian_batch_quat(T_flat.unsqueeze(0), *args)
            residual_eval_ms.append(float((_sync_now() - residual_t0) * 1e3))
            residual_cost_breakdown.append(dict(getattr(self.cost_fn, 'last_eval_timing', {})))
            ...
            linear_solve_ms.append(float((_sync_now() - solve_t0) * 1e3))
            ...
            T = T @ T_delta
            T_flat = T.to_flat()
            pose_update_ms.append(float((_sync_now() - update_t0) * 1e3))
            ...
            # 这里只在最后一次迭代后重算 post-update cost，避免每次迭代都重复前向。
            if i == num_iters - 1:
                with torch.no_grad():
                    post_cost_t0 = _sync_now()
                    ...
                    post_cost_ms.append(float((_sync_now() - post_cost_t0) * 1e3))
                    post_cost_breakdown.append(dict(getattr(self.cost_fn, 'last_eval_timing', {})))
                overall_losses.append(cost_post)
            iter_total_ms.append(float((_sync_now() - iter_t0) * 1e3))
        ...
        self.last_run_timing = {
            ...
            'residual_eval_ms': residual_eval_ms,
            'residual_cost_breakdown': residual_cost_breakdown,
            'linear_solve_ms': linear_solve_ms,
            'pose_update_ms': pose_update_ms,
            'post_cost_ms': post_cost_ms,
            'post_cost_breakdown': post_cost_breakdown,
            'iter_total_ms': iter_total_ms,
            'optimizer_total_ms': float((end_time - start_time) * 1e3),
        }
