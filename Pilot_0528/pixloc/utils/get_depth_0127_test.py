...
def generate_translation_seeds_ECEF(base_translation, dx_m=None, dy_m=None, dz_m=None, device='cuda'):
    """
    在 ECEF 坐标系下对平移（经纬高对应的 xyz）撒种子，写法与 generate_rotvecs_cuda_sym3d 一致：
    base_ECEF + meshgrid(dx_m, dy_m, dz_m)，偏移单位为米（ECEF 下 X/Y/Z 方向）。

    base_translation: (3,) 或 list [lon_deg, lat_deg, height_m] WGS84
    dx_m, dy_m, dz_m: 一维 tensor 或 list，ECEF 各轴偏移（米），None 则用默认 ±5m 三档
    返回: (N, 3) ECEF 位置，float32，在 device 上
    """
    base = torch.as_tensor(base_translation, device=device, dtype=torch.float32)
    if base.dim() == 0:
        base = base.unsqueeze(0).expand(3)
    base_ECEF = WGS84_to_ECEF_tensor(base, device=device)
    if base_ECEF.dim() == 1:
        base_ECEF = base_ECEF.unsqueeze(0)  # (1, 3) 方便后面广播
    else:
        base_ECEF = base_ECEF.reshape(1, 3)

    if dx_m is None:
        dx_m = torch.tensor([-5.0, 0.0, 5.0], device=device, dtype=torch.float32)
    else:
        dx_m = torch.as_tensor(dx_m, device=device, dtype=torch.float32)
    if dy_m is None:
        dy_m = torch.tensor([-5.0, 0.0, 5.0], device=device, dtype=torch.float32)
    else:
        dy_m = torch.as_tensor(dy_m, device=device, dtype=torch.float32)
    if dz_m is None:
        dz_m = torch.tensor([-5.0, 0.0, 5.0], device=device, dtype=torch.float32)
    else:
        dz_m = torch.as_tensor(dz_m, device=device, dtype=torch.float32)
    if dx_m.dim() == 0:
        dx_m = dx_m.unsqueeze(0)
    if dy_m.dim() == 0:
        dy_m = dy_m.unsqueeze(0)
    if dz_m.dim() == 0:
        dz_m = dz_m.unsqueeze(0)

    Dx, Dy, Dz = torch.meshgrid(dx_m, dy_m, dz_m, indexing='ij')
    n = Dx.numel()
    dx_flat = Dx.reshape(n)
    dy_flat = Dy.reshape(n)
    dz_flat = Dz.reshape(n)
    offsets = torch.stack([dx_flat, dy_flat, dz_flat], dim=1)
    translations_ECEF = base_ECEF.squeeze(0) + offsets
    b = base_ECEF.squeeze()
    print("[种子-ECEF] base_ECEF(m): %.2f, %.2f, %.2f | dx[%.1f, %.1f], dy[%.1f, %.1f], dz[%.1f, %.1f] m | N_trans=%d"
          % (b[0].item(), b[1].item(), b[2].item(),
             dx_m.min().item(), dx_m.max().item(),
             dy_m.min().item(), dy_m.max().item(),
             dz_m.min().item(), dz_m.max().item(), n))
    return translations_ECEF


def euler_angles_to_matrix_ECEF_batch_special_cuda_ECEF_trans(euler_angles, base_translation_wgs84, translations_ECEF, device='cuda'):
    """
    批量构造 ECEF 下 c2w：旋转由 euler + base_translation 的 ENU 得到，平移直接用 ECEF（米）。
    euler_angles: (N, 3) [pitch, roll, yaw] 度
    base_translation_wgs84: (3,) 仅用于 ENU 旋转
    translations_ECEF: (N, 3) 各候选 ECEF 位置（米）
    """
    ...
    R_batch = euler_to_rotm_batch(euler_angles, base_translation_wgs84, degrees=True, device=device)
    N = R_batch.shape[0]
    T = torch.eye(4, device=device, dtype=torch.float32).unsqueeze(0).repeat(N, 1, 1)
    T[:, :3, :3] = R_batch
    T[:, :3, 3] = translations_ECEF
    return T
...
def get_3D_samples_v3(mkpts_r, depth_mat, T_c2w, camera, euler_angles, translation,  query_euler_angles, query_translation, last_frame_info = {}, origin = None, device = 'cuda', num_init_pose = 32, T_query_in_ECEF_c2w = None, mul = None):
    ...
    query_translations_ECEF = generate_translation_seeds_ECEF(query_translation, device=device)

    N_euler = query_euler_angles.shape[0]
    N_trans = query_translations_ECEF.shape[0]
    euler_expanded = query_euler_angles.unsqueeze(1).expand(-1, N_trans, -1).reshape(N_euler * N_trans, 3)
    trans_expanded = query_translations_ECEF.unsqueeze(0).expand(N_euler, -1, -1).reshape(N_euler * N_trans, 3)
    query_T_c2w = euler_angles_to_matrix_ECEF_batch_special_cuda_ECEF_trans(
        euler_expanded, query_translation, trans_expanded, device=device
    )
