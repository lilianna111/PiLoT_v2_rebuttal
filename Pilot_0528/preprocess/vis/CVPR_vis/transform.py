...
def visualize_matches(img1, img2, points1, points2, 
    ...
    for i, ((x1, y1), (x2, y2)) in enumerate(zip(points1, points2)):
        ...
        if draw_lines:
            cv2.line(vis_img, (int(x1), int(y1)), (int(x2 + w1), int(y2)), color, 3)
...
def colmap_to_osg_bak(T_colmap_in_ned):
    ...
    return euler_angles, t_c2w, T_ECEF, T_ECEF_W2C
def calculate_matrix_from_precomputed_t(translation, euler_angles, t_c2w, origin=[0,0,0], mode='c2w'):
    """
    根据给定的原始经纬高、欧拉角和预先计算好的ECEF平移向量，来计算变换矩阵。
    """
    ...
    T_c2w = np.eye(4)
    T_c2w[:3, :3] = R_c2w
    T_c2w[:3, 3] = t_c2w  # 直接使用传入的、已批量计算好的 t_c2w
    T_c2w[:3, 3] -= origin
