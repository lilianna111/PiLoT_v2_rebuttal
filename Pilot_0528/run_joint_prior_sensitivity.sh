...
output_base=${OUTPUT_BASE:-/media/amax/PortableSSD/0908/Joint-ECEF-XY-Yaw}
...
# Baseline, followed by every combination of +/-ECEF-X or +/-ECEF-Y
# and +/-yaw at the paired 5 m/5 deg, 10 m/10 deg, and 15 m/15 deg levels.
dx_values=(0)
dy_values=(0)
yaw_values=(0)
for level in 5 10 15; do
  for yaw_sign in 1 -1; do
    yaw_deg=$((yaw_sign * level))
    dx_values+=("$level" "$((-level))" 0 0)
    dy_values+=(0 0 "$level" "$((-level))")
    yaw_values+=("$yaw_deg" "$yaw_deg" "$yaw_deg" "$yaw_deg")
  done
done
...
component_label() {
  ...
  local suffix=$3
  if (( value > 0 )); then
    printf "%sp%02d%s" "$prefix" "$value" "$suffix"
  elif (( value < 0 )); then
    printf "%sn%02d%s" "$prefix" "$((-value))" "$suffix"
  else
    printf "%s000%s" "$prefix" "$suffix"
  fi
}

manifest="$run_root/manifest.csv"
if [[ ! -e "$manifest" ]]; then
  echo "dx_m,dy_m,xy_magnitude_m,yaw_error_deg,yaw_magnitude_deg,sample_num,seed,sequence,status,output_folder" > "$manifest"
fi

for index in "${!dx_values[@]}"; do
  dx_m=${dx_values[$index]}
  dy_m=${dy_values[$index]}
  yaw_deg=${yaw_values[$index]}
  condition="$(component_label dx "$dx_m" m)_$(component_label dy "$dy_m" m)_$(component_label yaw "$yaw_deg" deg)"
  ...
  for sequence in "${sequences[@]}"; do
    ...
    echo "$dx_m,$dy_m,$magnitude_m,$yaw_deg,$magnitude_deg,$sample_num,$seed,$sequence,$status,$output_folder" >> "$manifest"
  done
done

if ! "$python_bin" summarize_joint_prior.py "$run_root"; then
  echo "Failed to summarize experiment: $run_root" >&2
  exit 1
fi

echo "Joint ECEF-XY/yaw experiment saved to: $run_root"
